...
int mspriorCode(int *prCoef, int *prGroup, struct marginalPars *pars) {
  ...
  if (*prCoef == 100) {
    ...
    if (!hasgroups) {
      ...
      if ((*prCoef == 0) & (*prGroup == 0)) {
        //pMOM on all coef
        ans = 0;
      } else if ((*prCoef == 1) & (*prGroup == 1)) {
        //piMOM on all coef
        ans = 1;
      } else if ((*prCoef == 2) & (*prGroup == 2)) {
        //peMOM on all coef
        ans = 2;
      } else if ((*prCoef == 3) & (*prGroup == 3)) {
        //Zellner on all coef
        ans = 3;
      } else if ((*prCoef == 4) & (*prGroup == 4)) {
        //normalid on all coef
        ans = 4;
      } else if ((*prCoef == 0) & (*prGroup == 10)) {
        //pMOM + group MOM
        ans = 10;
      } else if ((*prCoef == 0) & (*prGroup == 13)) {
        //pMOM + group Zellner
        ans = 13;
      } else if ((*prCoef == 2) & (*prGroup == 12)) {
        //peMOM + group eMOM
        ans = 32;
      } else if ((*prCoef == 2) & (*prGroup == 13)) {
        //peMOM + group Zellner
        ans = 33;
      } else if ((*prCoef == 3) & (*prGroup == 13)) {
        //Zellner + group Zellner
        ans = 43;
      } else if ((*prCoef == 10) & (*prGroup == 10)) {
        //group pMOM + group pMOM
        ans = 50;
      } else if ((*prCoef == 10) & (*prGroup == 13)) {
        //group pMOM + group Zellner
        ans = 53;
      } else if ((*prCoef == 13) & (*prGroup == 13)) {
        //group Zellner + group Zellner
        ans = 63;
      } else if ((*prCoef == 4) & (*prGroup == 13)) {
        //normalid + group Zellner
        ans = 73;
      } else {
...
pt2margFun set_marginalFunction(struct marginalPars *pars) {
  ...
  */
  ...
  if ((family) == 1) {
    ...
    //AFT survival with Normal errors
    if (!hasgroups) {
      if (priorcode == 0) {
      ...
      if (priorcode == 10) {
        ...
        if (*(*pars).method != 2)
          Rprintf(
            "For priorCoef=groupmomprior() + priorGroups=groupmomprior() only method='ALA' is implemented. Using ALA instead.\n");
      } else if (priorcode == 13) {
        ans = pmomgzellSurvMarg;
      } else if (priorcode == 32) {
        ...
        if (*(*pars).method != 2)
          Rprintf(
            "For priorCoef=groupmomprior() + priorGroups=groupzellnerprior() only method='ALA' is implemented. Using ALA instead.\n");
        ans = gmomgmomSurvMarg;
      } else if (priorcode == 53) {
        if (*(*pars).method != 2)
          Rprintf(
            "For priorCoef=groupmomprior() + priorGroups=groupzellnerprior() only method='ALA' is implemented. Using ALA instead. You can set method='Laplace' for priorCoef=momprior() + priorGroups=groupzellnerprior()\n");
    ...
    //Normal + Two-piece Normal + Laplace + Two-piece Laplace errors
...
pt2margFun set_priorFunction(int *prDelta, int *prConstr, int *family) {
  ...
  if (*family != 0) {
    if (*prDelta != *prConstr)
      Rf_error(
        "priorConstraints must be of the same family as priorDelta (e.g. both Binomial, both Beta-Binomial, both Complexity priors)");
    if (*prDelta == 0) { ans = unifPrior; } else if (*prDelta == 1) { ans = binomPrior; } else if (*prDelta == 2) {
      ans = betabinPrior;
    } else if (*prDelta == 3) { ans = complexityPrior; }
  } else {
    if (*prDelta == 0) { ans = unifPriorTP; } else if (*prDelta == 1) { ans = binomPriorTP; } else if (*prDelta == 2) {
      ans = betabinPriorTP;
    } else if (*prDelta == 3) { ans = complexityPrior; }
  }
  return ans;
...
pt2modavgPrior set_priorFunction_modavg(int *priorModel) {
  ...
  if (*priorModel == 0) { ans = unifPrior_modavg; } else if (*priorModel == 1) { ans = binomPrior_modavg; } else if (
    *priorModel == 2) { ans = betabinPrior_modavg; } else if (*priorModel == 3) { ans = complexityPrior_modavg; }
  return ans;
}
...
void set_modavgPars(struct modavgPars *pars, int *n, int *p1, int *p2, int *isbinary, int *ybinary, double *y,
                    double *sumy2, double *x1, double *x2, crossprodmat *XtX, double *ytX, double *cholS2,
                    double *S2inv, double *cholS2inv, double *colsumx1sq, double *alpha, double *lambda, int *priorCoef,
                    int *r, double *tau1, double *tau2, int *priorTau1, double *atau1, double *btau1, int *priorModel,
                    double *prModelpar) {
  (*pars).n          = n;
  (*pars).p1         = p1;
  (*pars).p2         = p2;
...
SEXP pmomLM_I(SEXP niter, SEXP thinning, SEXP burnin, SEXP niniModel, SEXP iniModel, SEXP iniCoef1, SEXP iniCoef2,
              SEXP iniPhi, SEXP iniOthers, SEXP verbose, SEXP n, SEXP p1, SEXP p2, SEXP isbinary, SEXP ybinary, SEXP y,
              SEXP sumy2, SEXP x1, SEXP x2, SEXP SXtX, SEXP ytX, SEXP cholS2, SEXP S2inv, SEXP cholS2inv,
              SEXP colsumx1sq, SEXP alpha, SEXP lambda, SEXP priorCoef, SEXP r, SEXP tau1, SEXP tau2, SEXP priorTau1,
              SEXP atau1, SEXP btau1, SEXP priorModel, SEXP prModelpar) {
  ...
  set_modavgPars(&pars, INTEGER(n), INTEGER(p1), INTEGER(p2), INTEGER(isbinary), INTEGER(ybinary), REAL(y), REAL(sumy2),
                 REAL(x1), REAL(x2), XtX, REAL(ytX), REAL(cholS2), REAL(S2inv), REAL(cholS2inv), REAL(colsumx1sq),
                 REAL(alpha), REAL(lambda), INTEGER(priorCoef), INTEGER(r), &tau1copy, REAL(tau2), INTEGER(priorTau1),
                 REAL(atau1), REAL(btau1), INTEGER(priorModel), REAL(prModelpar));
  pmomLM(postModel, margpp, postCoef1, postCoef2, postPhi, postOther, &pars, INTEGER(niter), INTEGER(thinning),
         INTEGER(burnin), INTEGER(niniModel), INTEGER(iniModel), REAL(iniCoef1), REAL(iniCoef2), REAL(iniPhi),
         REAL(iniOthers), INTEGER(verbose));
...
void pmomLM(int *postModel, double *margpp, double *postCoef1, double *postCoef2, double *postPhi, double *postOther,
            struct modavgPars *pars, int *niter, int *thinning, int *burnin, int *niniModel, int *iniModel,
            double *iniCoef1, double *iniCoef2, double *iniPhi, double *iniOthers, int *verbose) {
  int i, j, k, ilow, iupper, savecnt, niterthin, niter10, nsel = *niniModel, *curModel, newdelta, n = *(*pars).n, p1 = *
      (*pars).p1, p2 = *(*pars).p2, psn, resupdate, isbinary = *(*pars).isbinary;
  double *res, *partialres, sumres2, sumpartialres2, newcoef, *curCoef1, *curCoef2, curPhi = 1.0, *linpred1, *linpred2,
      pinclude, *temp;
  ...
  //Initialize
  curModel   = ivector(0, p1);
  curCoef1   = dvector(0, p1);
  curCoef2   = dvector(0, p2);
  linpred1   = dvector(0, n);
  linpred2   = dvector(0, n);
  res        = dvector(0, n);
  partialres = dvector(0, n);
  for (i = 0; i < p1; i++) {
    margpp[i]   = 0;
    curModel[i] = postModel[i * niterthin] = iniModel[i];
    curCoef1[i] = postCoef1[i * niterthin] = iniCoef1[i];
  }
  ...
  for (i = 0, sumres2 = 0; i < n; i++) {
    res[i] = partialres[i] = (*pars).y[i] - linpred1[i] - linpred2[i];
    sumres2 += res[i] * res[i];
  }
  ...
  for (i = ilow; i < iupper; i++) {
    ...
    for (j = 0; j < *(*pars).p1; j++) {
      if (curModel[j]) {
        for (k = 0, sumpartialres2 = 0; k < n; k++) {
          partialres[k] = res[k] + curCoef1[j] * ((*pars).x1[n * j + k]);
          sumpartialres2 += partialres[k] * partialres[k];
        }
      }
      MHTheta1pmom(&newdelta, &newcoef, &pinclude, &resupdate, res, partialres, &sumres2, &sumpartialres2, j, &nsel,
                   curModel, curCoef1, &curPhi, pars);
  ...
  if (iupper > ilow) { for (j = 0; j < p1; j++) { margpp[j] /= (iupper - imax_xy(0, ilow) + .0); } }
  //from sum to average
  if (*verbose == 1) Rprintf("Done.\n");
  free_ivector(curModel, 0, p1);
  free_dvector(curCoef1, 0, p1);
  free_dvector(curCoef2, 0, p2);
  free_dvector(linpred1, 0, n);
  free_dvector(linpred2, 0, n);
  free_dvector(res, 0, n);
  free_dvector(partialres, 0, n);
}
...
void sample_latentProbit(double *y, double *res, double *sumres2, int *ybinary, double *linpred1, double *linpred2,
                         struct modavgPars *pars) {
  int i;
...
void MHTheta1pmom(int *newdelta, double *newcoef, double *pinclude, int *resupdate, double *res, double *partialres,
                  double *sumres2, double *sumpartialres2, int j, int *nsel, int *curModel, double *curCoef1,
                  double *curPhi, struct modavgPars *pars) {
  int n = *(*pars).n, logscale = 1, nsel0, nsel1, deltaprop, nu, i;
  double m1, *xj, m0, logbf, logpratio, thetaprop, m, S, propPars[6], lhood, lprior, lprop, lambda = 0.0, num, den,
      sqrtPhi = sqrt(*curPhi);
  ...
  *resupdate = 0;
  ...
  //Propose delta
  if (curModel[j]) {
    m1 = pmomMargKuniv(partialres, xj, sumpartialres2, (*pars).colsumx1sq + j, &n, curPhi, (*pars).tau1, (*pars).r,
                       &logscale);
  ...
  logbf     = m0 - m1;
  logpratio = priorFunction(curModel, &nsel0, pars) - priorFunction(curModel, &nsel1, pars);
  //we use curModel in both cases as priorFunction currently only depends on nb vars
  ...
  if ((curModel[j] == 0) && (deltaprop == 0)) {
    //proposal is to keep variable out of the model
    *newdelta = 0;
    *newcoef  = 0;
  } else {
    ...
    if (curModel[j] && deltaprop) {
      //proposal is to keep variable in the model
      ...
      lprior = dmom(thetaprop, 0, *(*pars).tau1, *curPhi, *(*pars).r, 1) - dmom(
                 curCoef1[j], 0, *(*pars).tau1, *curPhi, *(*pars).r, 1);
      lprop = dtmixC(curCoef1[j], propPars, propPars + 2, propPars + 4, nu, 2, 1) - dtmixC(
                thetaprop, propPars, propPars + 2, propPars + 4, nu, 2, 1);
      lambda = exp(lhood + lprior + lprop);
    } else if ((curModel[j] == 0) && deltaprop) {
      //proposal is to add variable to the model
      thetaprop = rtmixC(propPars, propPars + 2, propPars + 4, nu, 2);
      for (i = 0, num = 0; i < n; i++) {
      ...
      //(curModel[j] && (deltaprop==0)), i.e. proposal is to drop variable from the model
      thetaprop = 0;
...
void proposalpmom(double *propPars, double *m, double *S, double *phi, int *r, double *tau1, int *n, double *e,
                  double *xj, double *m1, int *nu) {
  int i;
  ...
  //Proposal variances
  temp        = (*S) / (*phi);
  propPars[2] = sqrt(1.0 / (temp + doubler / (propPars[0] * propPars[0])));
  propPars[3] = sqrt(1.0 / (temp + doubler / (propPars[1] * propPars[1])));
  temp2       = .5 * (*nu);
  temp        = temp2 + .5;
  //Weights
...
double pmomMargKuniv(double *y, double *x, double *sumy2, double *sumxsq, int *n, double *phi, double *tau, int *r,
                     int *logscale) {
  int i;
  double ans, m, s, I, doubler = 2.0 * (*r);
  s                            = *sumxsq + 1.0 / (*tau);
  for (i = 0, m = 0; i < *n; i++) { m += y[i] * x[i]; }
  m /= s;
  I   = log(mnorm(doubler, m, sqrt(*phi / s)));
  ans = I - .5 * (*sumy2 - s * m * m) / (*phi) - .5 * (*n) * log(2 * M_PI * (*phi)) - .5 * (log(s) + log(*tau)) -
        ldoublefact(doubler - 1) - (*r) * log((*tau) * (*phi));
...
void simTheta2(double *theta2, double *partialres, double *phi, struct modavgPars *pars) {
  int i, j;
  double *tmp, *m, **cholS, sqrtPhi = sqrt(*phi);
  //m= S2inv * t(x2) * partialres
  tmp   = dvector(0, *(*pars).p2);
  m     = dvector(0, *(*pars).p2);
  cholS = dmatrix(1, *(*pars).p2, 1, *(*pars).p2);
  Atvecx((*pars).x2, partialres, tmp, 0, *(*pars).p2 - 1, 0, *(*pars).n - 1);
  Avecx((*pars).S2inv, tmp, m, 0, *(*pars).p2, 0, *(*pars).p2);
  //S= S2inv * phi
  for (i = 0; i < *(*pars).p2; i++) {
    for (j = 0; j < *(*pars).p2; j++) { cholS[i + 1][j + 1] = sqrtPhi * (*pars).cholS2inv[i + j * (*(*pars).p2)]; }
  }
  //Generate theta2 ~ N(m,S)
  rmvnormC(theta2 - 1, *(*pars).p2, m - 1, cholS);
  free_dvector(tmp, 0, *(*pars).p2);
  free_dvector(m, 0, *(*pars).p2);
  free_dmatrix(cholS, 1, *(*pars).p2, 1, *(*pars).p2);
}
...
void set_marginalPars(struct marginalPars *pars, int *family, int *n, int *nuncens, int *p, double *y, int *uncens,
                      double *sumy2, double *sumy, double *sumlogyfact, double *x, double *colsumsx, crossprodmat *XtX,
                      double *ytX, int *method, int *adjoverdisp, int *hesstype, int *optimMethod, int *optim_maxit,
                      int *usethinit, double *thinit, int *B, double *alpha, double *lambda, int *knownphi, double *phi,
                      double *tau, double *taugroup, double *taualpha, double *fixatanhalpha, int *r, double *prDeltap,
                      double *parprDeltap, double *prConstrp, double *parprConstrp, int *logscale, double *offset,
                      int *groups, int *isgroup, int *ngroups, int *ngroupsconstr, int *nvaringroup,
                      int *nconstraints = NULL, int *ninvconstraints = NULL, crossprodmat *XtXuncens = NULL,
                      double *ytXuncens = NULL) {
  (*pars).family          = family;
  (*pars).n               = n;
...
void set_f2int_pars(crossprodmat *XtX, double *ytX, double *tau, int *n, int *p, int *sel, int *nsel, double *y,
                    double *sumy2, int *method, int *B, double *alpha, double *lambda, int *logscale) {
  f2int_pars.XtX      = XtX;
  f2int_pars.ytX      = ytX;
  f2int_pars.tau      = tau;
  f2int_pars.n        = n;
...
// [[Rcpp::export]]
SEXP modelSelectionEnumCI(SEXP Snmodels, SEXP Smodels, SEXP Sknownphi, SEXP Sfamily, SEXP SpriorCoef, SEXP SpriorGroup,
                          SEXP Sn, SEXP Sp, SEXP Sy, SEXP Suncens, SEXP Ssumy2, SEXP Ssumy, SEXP Ssumlogyfact, SEXP Sx,
                          SEXP Scolsumsx, SEXP ShasXtX, SEXP SXtX, SEXP SytX, SEXP Smethod, SEXP Sadjoverdisp,
                          SEXP Shesstype, SEXP SoptimMethod, SEXP Soptim_maxit, SEXP Sthinit, SEXP Susethinit, SEXP SB,
                          SEXP Salpha, SEXP Slambda, SEXP Sphi, SEXP Stau, SEXP Staugroup, SEXP Staualpha,
                          SEXP Sfixatanhalpha, SEXP Sr, SEXP SpriorDelta, SEXP SprDeltap, SEXP SparprDeltap,
                          SEXP SpriorConstr, SEXP SprConstrp, SEXP SparprConstrp, SEXP Sgroups, SEXP Sngroups,
                          SEXP Snvaringroup, SEXP Sconstraints, SEXP Sinvconstraints, SEXP Sverbose) {
  bool hasXtX              = LOGICAL(ShasXtX)[0];
  int i, j, idxj, logscale = 1, *postMode, mycols, mycols2, nuncens, *nconstraints, *ninvconstraints, ngroupsconstr = 0,
      *isgroup, usethinit  = INTEGER(Susethinit)[0], priorcode;
  ...
  if (INTEGER(Sfamily)[0] != 0) { mycols = mycols2 = INTEGER(Sp)[0]; } else {
    mycols  = 2 + INTEGER(Sp)[0];
    mycols2 = mycols + 2;
  }
  ...
  ninvconstraints = ivector(0, INTEGER(Sngroups)[0]);
  countConstraints(nconstraints, &constraints, ninvconstraints, &invconstraints, &ngroupsconstr, isgroup,
                   INTEGER(Sngroups), INTEGER(Snvaringroup), Sconstraints, Sinvconstraints);
  ...
  if (LENGTH(Suncens) > 0) {
    ...
    for (j = 0; j < INTEGER(Sp)[0]; j++) {
      for (i = 0, ytXuncens[j] = 0, idxj = j * n; i < nuncens; i++) { ytXuncens[j] += pty[i] * ptx[i + idxj]; }
    }
  } else { nuncens = INTEGER(Sn)[0]; }


  set_marginalPars(&pars, INTEGER(Sfamily), INTEGER(Sn), &nuncens, INTEGER(Sp), REAL(Sy), INTEGER(Suncens),
                   REAL(Ssumy2), REAL(Ssumy), REAL(Ssumlogyfact), REAL(Sx), REAL(Scolsumsx), XtX, REAL(SytX),
                   INTEGER(Smethod), INTEGER(Sadjoverdisp), INTEGER(Shesstype), INTEGER(SoptimMethod),
                   INTEGER(Soptim_maxit), &usethinit, thinit, INTEGER(SB), REAL(Salpha), REAL(Slambda),
                   INTEGER(Sknownphi), REAL(Sphi), REAL(Stau), REAL(Staugroup), REAL(Staualpha), REAL(Sfixatanhalpha),
                   INTEGER(Sr), REAL(SprDeltap), REAL(SparprDeltap), REAL(SprConstrp), REAL(SparprConstrp), &logscale,
                   &offset, INTEGER(Sgroups), isgroup, INTEGER(Sngroups), &ngroupsconstr, INTEGER(Snvaringroup),
                   nconstraints, ninvconstraints, XtXuncens, ytXuncens);

  priorcode      = mspriorCode(INTEGER(SpriorCoef), INTEGER(SpriorGroup), &pars);
  pars.priorcode = &priorcode;

  modelSelectionEnum(postMode, postModeProb, postProb, INTEGER(Snmodels), INTEGER(Smodels), INTEGER(SpriorDelta),
                     INTEGER(SpriorConstr), INTEGER(Sverbose), &pars);

  delete XtX;
  free_ivector(nconstraints, 0, INTEGER(Sngroups)[0]);
  free_ivector(ninvconstraints, 0, INTEGER(Sngroups)[0]);
  free_dvector(thinit, 0, mycols2 + 1);
  free_ivector(isgroup, 0, INTEGER(Sp)[0]);
  if (LENGTH(Suncens) > 0) {
    delete XtXuncens;
    free_dvector(ytXuncens, 0, INTEGER(Sp)[0]);
  }
  UNPROTECT(1);
  return ans;
}
...
void modelSelectionEnum(int *postMode, double *postModeProb, double *postProb, int *nmodels, int *models, int *prDelta,
                        int *prConstr, int *verbose, struct marginalPars *pars) {
  ...
  mfamily = dvector(0, nbfamilies - 1);
  pfamily = dvector(0, nbfamilies - 1);
  if ((*family) == 0) {
    //infer error distribution (Normal/Asymmetric Normal/Laplace/Asymmetric Laplace)
    nbvars    = (*(*pars).p) + 1;
  ...
  for (i = 0; i < *nmodels; i++) {
    ...
    if (nsel <= (*(*pars).n)) {
      if ((*family) == 0) {
        //inference is being done on the family
      ...
      if (postProb[i] > *postModeProb) {
        (*postModeProb) = postProb[i];
        postModeidx     = i;
      }
    }
  ...
  free_dvector(mfamily, 0, nbfamilies - 1);
  free_dvector(pfamily, 0, nbfamilies - 1);
  delete integrals;
}
...
// [[Rcpp::export]]
SEXP modelSelectionGibbsCI(SEXP SpostModeini, SEXP SpostModeiniProb, SEXP Sknownphi, SEXP Sfamily, SEXP SpriorCoef,
                           SEXP SpriorGroup, SEXP Sniter, SEXP Sthinning, SEXP Sburnin, SEXP Sndeltaini, SEXP Sdeltaini,
                           SEXP Sincludevars, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Suncens, SEXP Ssumy2, SEXP Ssumy,
                           SEXP Ssumlogyfact, SEXP Sx, SEXP Scolsumsx, SEXP ShasXtX, SEXP SXtX, SEXP SytX, SEXP Smethod,
                           SEXP Sadjoverdisp, SEXP Shesstype, SEXP SoptimMethod, SEXP Soptim_maxit, SEXP Sthinit,
                           SEXP Susethinit, SEXP SB, SEXP Salpha, SEXP Slambda, SEXP Sphi, SEXP Stau, SEXP Staugroup,
                           SEXP Staualpha, SEXP Sfixatanhalpha, SEXP Sr, SEXP SpriorDelta, SEXP SprDeltap,
                           SEXP SparprDeltap, SEXP SpriorConstr, SEXP SprConstrp, SEXP SparprConstrp, SEXP Sgroups,
                           SEXP Sngroups, SEXP Snvaringroup, SEXP Sconstraints, SEXP Sinvconstraints, SEXP Sverbose) {
  bool hasXtX                = LOGICAL(ShasXtX)[0];
  int i, j, idxj, logscale   = 1, mcmc2save, *postSample, *postMode, mycols, mycols2, *nconstraints, *ninvconstraints,
      nuncens, ngroupsconstr = 0, *isgroup, usethinit                           = INTEGER(Susethinit)[0], priorcode;
  ...
  Rcout << "--- Parameters for modelSelectionGibbs ---" << endl;

  Rcout << "postSample (first 10 elements): ";
  for (int k = 0; k < min(10, mcmc2save * mycols); ++k) {
    Rcout << postSample[k] << " ";
  }
  Rcout << (mcmc2save * mycols > 10 ? "..." : "") << endl;
  Rcout << "Dimensions of postSample: " << mcmc2save << " x " << mycols << endl;
  ...
  Rcout << endl;
  Rcout << "Length of margpp: " << mycols2 << endl;
  ...
  Rcout << endl;
  Rcout << "Length of postMode: " << mycols << endl;

  Rcout << "postModeProb: " << postModeProb[0] << endl;

  Rcout << "postProb (first 10 elements): ";
  for (int k = 0; k < min(10, mcmc2save); ++k) {
    Rcout << postProb[k] << " ";
  }
  Rcout << (mcmc2save > 10 ? "..." : "") << endl;
  Rcout << "Length of postProb: " << mcmc2save << endl;

  Rcout << "priorDelta: " << INTEGER(SpriorDelta)[0] << endl;
  Rcout << "priorConstr: " << INTEGER(SpriorConstr)[0] << endl;
  Rcout << "niter: " << INTEGER(Sniter)[0] << endl;
  Rcout << "thinning: " << INTEGER(Sthinning)[0] << endl;
  Rcout << "burnin: " << INTEGER(Sburnin)[0] << endl;
  Rcout << "ndeltaini: " << INTEGER(Sndeltaini)[0] << endl;

  Rcout << "deltaini: ";
  IntegerVector deltaini_vec(Sdeltaini);
  for (int k = 0; k < deltaini_vec.length(); ++k) {
    Rcout << deltaini_vec[k] << " ";
  }
  Rcout << endl;
  Rcout << "Length of deltaini: " << deltaini_vec.length() << endl;

  Rcout << "includevars: ";
  IntegerVector includevars_vec(Sincludevars);
  for (int k = 0; k < includevars_vec.length(); ++k) {
    Rcout << includevars_vec[k] << " ";
  }
  Rcout << endl;
  Rcout << "Length of includevars: " << includevars_vec.length() << endl;

  Rcout << "constraints (first level pointers): " << constraints.size() << endl;
  ...
  Rcout << "--- Parameters of pars struct ---" << endl;
  Rcout << "pars.family: " << *(pars.family) << endl;
  Rcout << "pars.n: " << *(pars.n) << endl;
  Rcout << "pars.nuncens: " << *(pars.nuncens) << endl;
  Rcout << "pars.p: " << *(pars.p) << endl;
  Rcout << "pars.y (first 10 elements): ";
  for (int k = 0; k < min(10, *(pars.n)); ++k) Rcout << pars.y[k] << " ";
  Rcout << (*(pars.n) > 10 ? "..." : "") << endl;
  Rcout << "pars.uncens (first 10 elements): ";
  if (pars.uncens != NULL) {
    for (int k = 0; k < min(10, *(pars.n)); ++k) Rcout << pars.uncens[k] << " ";
    Rcout << (*(pars.n) > 10 ? "..." : "") << endl;
  } else {
    Rcout << "NULL" << endl;
  }
  Rcout << "pars.sumy2: " << *(pars.sumy2) << endl;
  Rcout << "pars.sumy: " << *(pars.sumy) << endl;
  Rcout << "pars.sumlogyfact: " << *(pars.sumlogyfact) << endl;
  Rcout << "pars.x (first 10 elements): ";
  for (int k = 0; k < min(10, (*(pars.n) * *(pars.p))); ++k) Rcout << pars.x[k] << " ";
  Rcout << ((*(pars.n) * *(pars.p)) > 10 ? "..." : "") << endl;
  Rcout << "pars.colsumsx (first " << *(pars.p) << " elements): ";
  for (int k = 0; k < *(pars.p); ++k) Rcout << pars.colsumsx[k] << " ";
  Rcout << endl;
  Rcout << "pars.XtX: (printing address) " << pars.XtX << endl;
  Rcout << "pars.ytX (first " << *(pars.p) << " elements): ";
  for (int k = 0; k < *(pars.p); ++k) Rcout << pars.ytX[k] << " ";
  Rcout << endl;
  Rcout << "pars.method: " << *(pars.method) << endl;
  Rcout << "pars.adjoverdisp: " << *(pars.adjoverdisp) << endl;
  Rcout << "pars.hesstype: " << *(pars.hesstype) << endl;
  Rcout << "pars.optimMethod: " << *(pars.optimMethod) << endl;
  Rcout << "pars.optim_maxit: " << *(pars.optim_maxit) << endl;
  Rcout << "pars.usethinit: " << *(pars.usethinit) << endl;
  Rcout << "pars.thinit (first 10 elements): ";
  for (int k = 0; k < min(10, mycols2 + 2); ++k) Rcout << pars.thinit[k] << " ";
  Rcout << (mycols2 + 2 > 10 ? "..." : "") << endl;
  Rcout << "pars.B: " << *(pars.B) << endl;
  Rcout << "pars.alpha: " << *(pars.alpha) << endl;
  Rcout << "pars.lambda: " << *(pars.lambda) << endl;
  Rcout << "pars.knownphi: " << *(pars.knownphi) << endl;
  Rcout << "pars.phi: " << *(pars.phi) << endl;
  Rcout << "pars.tau: " << *(pars.tau) << endl;
  Rcout << "pars.taugroup: " << *(pars.taugroup) << endl;
  Rcout << "pars.taualpha: " << *(pars.taualpha) << endl;
  Rcout << "pars.fixatanhalpha: " << *(pars.fixatanhalpha) << endl;
  Rcout << "pars.r: " << *(pars.r) << endl;
  Rcout << "pars.prDeltap: " << *(pars.prDeltap) << endl;
  Rcout << "pars.parprDeltap: " << *(pars.parprDeltap) << endl;
  Rcout << "pars.prConstrp: " << *(pars.prConstrp) << endl;
  Rcout << "pars.parprConstrp: " << *(pars.parprConstrp) << endl;
  Rcout << "pars.logscale: " << *(pars.logscale) << endl;
  Rcout << "pars.offset: " << *(pars.offset) << endl;
  Rcout << "pars.groups (first " << *(pars.ngroups) << " elements): ";
  if (pars.groups != NULL) {
    for (int k = 0; k < *(pars.ngroups); ++k) Rcout << pars.groups[k] << " ";
    Rcout << endl;
  } else {
    Rcout << "NULL" << endl;
  }
  Rcout << "pars.isgroup (first " << *(pars.p) << " elements): ";
  for (int k = 0; k < *(pars.p); ++k) Rcout << pars.isgroup[k] << " ";
  Rcout << endl;
  Rcout << "pars.ngroups: " << *(pars.ngroups) << endl;
  Rcout << "pars.ngroupsconstr: " << *(pars.ngroupsconstr) << endl;
  Rcout << "pars.nvaringroup (first " << *(pars.ngroups) << " elements): ";
  for (int k = 0; k < *(pars.ngroups); ++k) Rcout << pars.nvaringroup[k] << " ";
  Rcout << endl;
  Rcout << "pars.nconstraints (first " << *(pars.ngroups) << " elements): ";
  for (int k = 0; k < *(pars.ngroups); ++k) Rcout << pars.nconstraints[k] << " ";
  Rcout << endl;
  Rcout << "pars.ninvconstraints (first " << *(pars.ngroups) << " elements): ";
  for (int k = 0; k < *(pars.ngroups); ++k) Rcout << pars.ninvconstraints[k] << " ";
  Rcout << endl;
  Rcout << "pars.XtXuncens: (printing address) " << pars.XtXuncens << endl;
  Rcout << "pars.ytXuncens (first " << *(pars.p) << " elements): ";
  if (pars.ytXuncens != NULL) {
    for (int k = 0; k < *(pars.p); ++k) Rcout << pars.ytXuncens[k] << " ";
    Rcout << endl;
  } else {
    Rcout << "NULL" << endl;
  }
  ...
  modelSelectionGibbs(postSample, margpp, postMode, postModeProb, postProb, INTEGER(SpriorDelta), INTEGER(SpriorConstr),
                      INTEGER(Sniter), INTEGER(Sthinning), INTEGER(Sburnin), INTEGER(Sndeltaini), INTEGER(Sdeltaini),
                      INTEGER(Sincludevars), &constraints, &invconstraints, INTEGER(Sverbose), &pars);
...
void modelSelectionGibbs(int *postSample, double *margpp, int *postMode, double *postModeProb, double *postProb,
                         int *prDelta, int *prConstr, int *niter, int *thinning, int *burnin, int *ndeltaini,
                         int *deltaini, int *includevars, intptrvec *constraints, intptrvec *invconstraints,
                         int *verbose, struct marginalPars *pars) {
  bool copylast, validmodel, nonbinary = false;
  int i, j, jgroup, jgroup2, k, niter10, niterthin, savecnt, ilow, iupper, itercount, nbvars, nbfamilies = 4, curfamily,
      newfamily, ngroups, *nvaringroup, *firstingroup, *nconstraints = (*pars).nconstraints, *ninvconstraints = (*pars).
          ninvconstraints, *family = (*pars).family;
  ...
  addgroups  = ivector(0, 1);
  dropgroups = ivector(0, 1);
  newJ       = dvector(0, 2);
  ppnew      = dvector(0, 3);
  modelidx   = ivector(0, 3);
  ...
  sel    = ivector(0, nbvars);
  selnew = ivector(0, nbvars);
  if (nonbinary) {
    selnew2 = ivector(0, nbvars);
    selnew3 = ivector(0, nbvars);
  }
  ...
  for (i = ilow, itercount = 0; i < iupper; i++, itercount++) {
    ...
    while (j < *(*pars).p) {
      if (sample[jgroup] > 0) {
        //if jgroup should be sampled

        sel2selnew(jgroup, sel, &nsel, selnew, &nselnew, copylast, &ngroups, nvaringroup, firstingroup);
        //copy sel into selnew, adding/removing jth group
        if (nsel > nselnew) {
          naddgroups    = 0;
          ndropgroups   = 1;
          dropgroups[0] = jgroup;
        ...
        validmodel = checkConstraints(addgroups, &naddgroups, dropgroups, &ndropgroups, constraints, nconstraints,
                                      invconstraints, ninvconstraints, (*pars).groups, nvaringroup, sel, &nsel);
        if (nselnew > (*(*pars).n)) validmodel = false;

        ppnew[0] = ppnewsum = 1;
        if (includevars[j] == 0 && validmodel) {
          //if proposed model is valid
          if ((*family) == 0) {
            nselplus1 = nselnew + 1;
            newJ[0]   = integrals->getJoint(selnew, &nselplus1, pars);
          } else {
            newJ[0] = integrals->getJoint(selnew, &nselnew, pars);
          }
          if (newJ[0] > *postModeProb) {
            *postModeProb = newJ[0];
            update_postMode(postMode, nselnew, selnew, *(*pars).p, *family);
        ...
        if ((nonbinary) && sample[jgroup] == 2) {
          //non-binary update

          jgroup2 = (*invconstraints)[jgroup][0];
          sel2selnew(jgroup2, sel, &nsel, selnew2, &nselnew2, copylast, &ngroups, nvaringroup, firstingroup);
          //Model changing inclusion/exclusion of jgroup2

          sel2selnew(jgroup2, selnew, &nselnew, selnew3, &nselnew3, copylast, &ngroups, nvaringroup, firstingroup);
          //Model changing inclusion/exclusion of (jgroup,jgroup2)
          if ((nsel > nselnew) && (nselnew > nselnew3)) {
            //nselnew3 dropped jgroup, dropped jgroup2
            modelidx[0] = 2;
            modelidx[1] = -1;
            modelidx[2] = 1;
            modelidx[3] = 0;
          } else if ((nsel > nselnew) && (nselnew < nselnew3)) {
            //nselnew3 dropped jgroup, added jgroup2
            modelidx[0] = 1;
            modelidx[1] = 0;
            modelidx[2] = 2;
            modelidx[3] = -1;
            ...
            //nselnew3 added jgroup, added jgroup2
            modelidx[0] = 0;
            modelidx[1] = 1;
            modelidx[2] = -1;
            modelidx[3] = 2;
          ...
          if ((modelidx[2] != -1) && (nselnew2 < (*(*pars).n))) {
            if ((*family) == 0) {
              nselplus1 = nselnew2 + 1;
              newJ[1]   = integrals->getJoint(selnew2, &nselplus1, pars);
            } else { newJ[1] = integrals->getJoint(selnew2, &nselnew2, pars); }
            if (newJ[1] > *postModeProb) {
              *postModeProb = newJ[1];
              update_postMode(postMode, nselnew2, selnew2, *(*pars).p, *family);
            }
          ...
          if ((modelidx[3] != -1) && (nselnew3 < (*(*pars).n))) {
            if ((*family) == 0) {
              nselplus1 = nselnew3 + 1;
              newJ[2]   = integrals->getJoint(selnew3, &nselplus1, pars);
            } else { newJ[2] = integrals->getJoint(selnew3, &nselnew3, pars); }
            if (newJ[2] > *postModeProb) {
              *postModeProb = newJ[2];
              update_postMode(postMode, nselnew3, selnew3, *(*pars).p, *family);
            }
          ...
          if (includevars[j] == 0 && validmodel) {
            ...
            ppnew[1] /= ppnewsum;
            if (i >= 0) { if (nselnew > nsel) { margpp[j] += ppnew[1]; } else { margpp[j] += (1 - ppnew[1]); } }
            ...
            if ((i >= 0) && (ndropgroups > 0) && (!validmodel)) { margpp[j] += 1.0; }
            //conditional marginal inclusion prob=1
          }
        }
    ...
    if ((i > 0) && ((i % (*thinning)) == 0)) {
      ...
      if ((*family) == 0) {
        ...
        } else if (sel[nsel] == (*(*pars).p) + 1) {
          //Asymmetric Normal residuals
          postSample[(*(*pars).p) * niterthin + savecnt]       = 1;
          postSample[((*(*pars).p + 1)) * niterthin + savecnt] = 0;
        } else if (sel[nsel] == (*(*pars).p) + 2) {
          //Laplace residuals
          postSample[(*(*pars).p) * niterthin + savecnt]       = 0;
          postSample[((*(*pars).p + 1)) * niterthin + savecnt] = 1;
        } else {
          //Asymmetric Laplace residuals
          postSample[(*(*pars).p) * niterthin + savecnt]       = 1;
          postSample[((*(*pars).p + 1)) * niterthin + savecnt] = 1;
        }
      }
  ...
  if (iupper > ilow) {
    for (j = 0; j < (*(*pars).p); j++) { margpp[j] /= (iupper - imax_xy(0, ilow) + .0); }
  } //from sum to average
  if (*family == 0) {
    margpp[(*(*pars).p)] /= (iupper - imax_xy(0, ilow) + .0);
    margpp[(*(*pars).p) + 1] /= (iupper - imax_xy(0, ilow) + .0);
    margpp[(*(*pars).p) + 2] /= (iupper - imax_xy(0, ilow) + .0);
    margpp[(*(*pars).p) + 3] /= (iupper - imax_xy(0, ilow) + .0);
  }
  ...
  free_ivector(modelidx, 0, 3);
  free_ivector(firstingroup, 0, ngroups);
  free_ivector(sel, 0, nbvars);
  free_ivector(selnew, 0, nbvars);
  free_ivector(sample, 0, ngroups - 1);
  if (nonbinary) {
    free_ivector(selnew2, 0, nbvars);
    free_ivector(selnew3, 0, nbvars);
  }
  free_dvector(mfamily, 0, nbfamilies - 1);
  free_dvector(pfamily, 0, nbfamilies - 1);
  delete integrals;
...
// [[Rcpp::export]]
SEXP greedyVarSelCI(SEXP Sknownphi, SEXP Sfamily, SEXP SpriorCoef, SEXP SpriorGroup, SEXP Sniter, SEXP Sndeltaini,
                    SEXP Sdeltaini, SEXP Sincludevars, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Suncens, SEXP Ssumy2, SEXP Ssumy,
                    SEXP Ssumlogyfact, SEXP Sx, SEXP Scolsumsx, SEXP ShasXtX, SEXP SXtX, SEXP SytX, SEXP Smethod,
                    SEXP Sadjoverdisp, SEXP Shesstype, SEXP SoptimMethod, SEXP Soptim_maxit, SEXP Sthinit,
                    SEXP Susethinit, SEXP SB, SEXP Salpha, SEXP Slambda, SEXP Sphi, SEXP Stau, SEXP Staugroup,
                    SEXP Staualpha, SEXP Sfixatanhalpha, SEXP Sr, SEXP SpriorDelta, SEXP SprDeltap, SEXP SparprDeltap,
                    SEXP SpriorConstr, SEXP SprConstrp, SEXP SparprConstrp, SEXP Sgroups, SEXP Sngroups,
                    SEXP Snvaringroup, SEXP Sconstraints, SEXP Sinvconstraints, SEXP Sverbose) {
  bool hasXtX              = LOGICAL(ShasXtX)[0];
  int i, j, idxj, logscale = 1, mycols, *postMode, *nconstraints, *ninvconstraints, nuncens, ngroupsconstr = 0, *isgroup
      , usethinit          = INTEGER(Susethinit)[0], priorcode;
  ...
  greedyVarSelC(postMode, postModeProb, INTEGER(SpriorDelta), INTEGER(SpriorConstr), INTEGER(Sniter),
                INTEGER(Sndeltaini), INTEGER(Sdeltaini), INTEGER(Sincludevars), &constraints, &invconstraints,
                INTEGER(Sverbose), &pars);

  free_dvector(thinit, 0, mycols + 1);
  free_ivector(isgroup, 0, INTEGER(Sp)[0]);
  free_ivector(nconstraints, 0, INTEGER(Sngroups)[0]);
  free_ivector(ninvconstraints, 0, INTEGER(Sngroups)[0]);
  delete XtX;
...
void greedyVarSelC(int *postMode, double *postModeProb, int *prDelta, int *prConstr, int *niter, int *ndeltaini,
                   int *deltaini, int *includevars, intptrvec *constraints, intptrvec *invconstraints, int *verbose,
                   struct marginalPars *pars) {
  int i, j, k, l, jgroup, *sel, *selnew, *selaux, nsel, nselnew, nchanges, ngroups, *nvaringroup, *firstingroup, *
      nconstraints = (*pars).nconstraints, *family = (*pars).family;
  double newJ;
  pt2margFun marginalFunction = NULL, priorFunction = NULL;
  //same as double (*marginalFunction)(int *, int *, struct marginalPars *);
  ...
  marginalFunction = set_marginalFunction(pars);
  priorFunction    = set_priorFunction(prDelta, prConstr, family);
  sel              = ivector(0, *(*pars).p);
  selnew           = ivector(0, *(*pars).p);
  ...
  *postModeProb = marginalFunction(sel, &nsel, pars) + priorFunction(sel, &nsel, pars);
  ...
  for (i = 0, nchanges = 1; (i < *niter) && (nchanges > 0); i++) {
    ...
    while (j < *(*pars).p) {
      sel2selnew(jgroup, sel, &nsel, selnew, &nselnew, false, &ngroups, nvaringroup, firstingroup);
      //copy sel into selnew, adding/removing jth group
      ...
      if (includevars[j] == 0) {
        ...
        if (newJ > *postModeProb) {
          *postModeProb = newJ; //update post mode prob
          ...
          sel           = selnew;
          selnew        = selaux;
          nsel          = nselnew; //update model indicator
          nchanges++;
        }
      }
      j += nvaringroup[jgroup];
      ...
      if ((postMode[firstingroup[j]] == 1) && (nconstraints[j] > 0)) {
        for (i = 0; i < nconstraints[j]; i++) {
          ...
          if (postMode[firstingroup[k]] == 0) {
            nchanges++;
            for (l = 0; l < nvaringroup[k]; l++) {
              postMode[firstingroup[k] + l] = 1;
              nselnew += nvaringroup[k];
            }
          }
        }
      }
  ...
  if (nselnew > nsel) {
    for (j = 0, i = 0; (j < *(*pars).p) && (i < nselnew); j++) {
      if (postMode[j] == 1) {
        selnew[i] = j;
        i++;
      }
    } //copy postMode into selnew
  ...
  free_ivector(firstingroup, 0, ngroups);
  free_ivector(sel, 0, *(*pars).p);
  free_ivector(selnew, 0, *(*pars).p);
}
...
void update_postMode(int *postMode, int nselnew, int *selnew, int p, int family) {
  ...
  if (family == 0) {
    if (selnew[nselnew] == p) {
      ...
      postMode[p]     = 0;
      postMode[p + 1] = 0;
    } else if (selnew[nselnew] == p + 1) {
      //Asymmetric Normal residuals
      postMode[p]     = 1;
      postMode[p + 1] = 0;
    } else if (selnew[nselnew] == (p) + 2) {
      //Laplace residuals
      postMode[p]     = 0;
      postMode[p + 1] = 1;
    } else {
  ...
  for (k = 0; (k < *naddgroups) && valid; k++) {
    ...
    while ((l < nconstraints[curgroup]) && (j < *nsel) && valid) {
      if (groups[sel[j]] > curconstraints[l]) {
        ...
        //add groups and nvaringroup as parameters
        l++;
        nvalid++;
      } else {
        j += nvaringroup[sel[j]];
...
void findselgroups(double *nvarinselgroups, double *firstingroup, double *nselgroups, double *selgroups, int *sel,
                   int *nsel, int *nvaringroup, int *ngroups) {
  ...
  while (j < *nsel) {
    ...
    if (nselgroupsi == 0) { firstingroup[0] = 0; } else {
      firstingroup[nselgroupsi] = firstingroup[nselgroupsi - 1] + nvarinselgroups[nselgroupsi - 1];
    }
...
void nselConstraints(int *ngroups0, int *ngroups1, int *sel, int *nsel, int *group, int *nconstraints,
                     int *nvaringroup) {
...
void countConstraints(int *nconstraints, intptrvec *constraints, int *ninvconstraints, intptrvec *invconstraints,
                      int *ngroupsconstr, int *isgroup, int *ngroups, int *nvaringroup, SEXP Sconstraints,
                      SEXP Sinvconstraints) {
...
double pmompenalty_approx(double *thopt, double **Hinv, double *tau, int thlength, double *nvaringroups,
                          double *firstingroup) {
  int i, idx;
...
double gmompenalty_approx(bool momsingle, bool momgroup, double *thopt, double **Hinv, double *Sinv, double phi,
                          int thlength, int nsel, int nselgroups, double *nvaringroups, double *firstingroup,
                          double *cholSini) {
  ...
  for (groupid = 0; groupid < nselgroups; groupid++) {
    ...
    if (((ningroup == 1) && momsingle) || ((ningroup > 1) && momgroup)) {
      //if MOM prior was set on the current group
...
double vectBinom(int *sel, int *nsel, int len_prDeltap, int len_prConstrp, struct marginalPars *pars) {
  int i, sel_i = 0, delta_i = 0, constr_i = 0, ngroups = *(*pars).ngroups, *groups = (*pars).groups, *nconstraints = (*
    pars).nconstraints, *nvaringroup = (*pars).nvaringroup;
...
double betabinPrior(int *sel, int *nsel, struct marginalPars *pars) {
  ...
  if ((*(*pars).ngroupsconstr) > 0)
    ans += bbPrior(ngroups1, *(*pars).ngroupsconstr, (*pars).parprConstrp[0],
                   (*pars).parprConstrp[1], 1);
  return ans;
...
void dmomgzell(double *ans, double *th, double *tau, double *nvaringroup, double *ngroups, double *ldetSinv,
               double *cholSinv, double *cholSini, bool logscale) {
...
void demomgzell(double *ans, double *th, double *tau, double *nvaringroup, double *ngroups, double *ldetSinv,
                double *cholSinv, double *cholSini, bool logscale) {
...
void dgzellgzell(double *ans, double *th, double *nvaringroup, double *ngroups, double *ldetSinv, double *cholSinv,
                 double *cholSini, bool logscale) {
...
*/
void gzell_Sinv(double *Sinv, double *cholSinv, double *ldetSinv, int *ngroups, double *nvaringroups, int *sel,
                double *cholSini, crossprodmat *XtX, double *tau, double *taugroup, bool orthoapprox) {
  bool posdef;
  ...
  for (i = 0, firstingroup = 0; i < *ngroups; i++) {
    ...
    idxini    = sel[firstingroup];
    Sidxini   = (int) (cholSini[i] + .1);
...
//    63: group Zellner + group Zellner
void gzell_Sinv_byprior(double *Sinv, double *cholSinv, double *ldetSinv, int *ngroups, double *nvaringroups, int *sel,
                        double *cholSini, crossprodmat *XtX, int *n, double *tau, double *taugroup, int *priorcode) {
  bool posdef, idcov, groupmom, groupzell;
  int i, j, l, k, firstingroup, groupsize, idxini, Sidxini;
  double ct, sqrtct, ct1 = 1, tauinv = 1, sqrttauinv = 1, neglogtau = 0;

  idcov = ((*priorcode == 0) || (*priorcode == 10) || (*priorcode == 13));
  //is the covariance for individual coef tau * 1
  groupmom  = ((*priorcode == 10) || (*priorcode == 50)); //was a group MOM prior set on groups
  groupzell = ((*priorcode == 9) || (*priorcode == 13) || (*priorcode == 43) || (*priorcode == 53) || (
                 *priorcode == 63)); //was a group Zellner set on groups
  ...
  if ((*priorcode == 9) || (*priorcode == 43) || (*priorcode == 63)) ct1 = 1.0 / (*tau);
  //group Zellner on individual coef
  if ((*priorcode == 5) || (*priorcode == 50) || (*priorcode == 53)) ct1 = 3.0 / ((*n) * (*tau));
  //group pMOM on individual coef
...
void pmomgzell_log(double *f, double *th, int *sel, int *thlength, struct marginalPars *pars,
                   std::map<string, double *> *funargs) {
  dmomgzell(f, th, (*pars).tau, (*funargs)["nvarinselgroups"], (*funargs)["nselgroups"], (*funargs)["ldetSinv"],
            (*funargs)["cholSinv"], (*funargs)["cholSini"], true);
  (*f) = -(*f);
}

//Gradient wrt th[j] of log pMOM + group Zellner prior
void pmomgzell_grad(double *priorgrad, int j, double *th, int *sel, int *thlength, struct marginalPars *pars,
                    std::map<string, double *> *funargs) {
  if (((*pars).isgroup)[sel[j]] == 0) {
    ...
    for (l = 0, (*priorgrad) = 0; l < jingroup; l++) {
      ll = idxini + l * ningroup - l * (l - 1) / 2;
      (*priorgrad) += Sinv[ll + jingroup - l] * th[firstingroup + l];
    } //Sinv[jingroup,] %*% th
...
void pmomgzell_gradhess(double *priorgrad, double *priorhess, int j, double *th, int *sel, int *thlength,
                        struct marginalPars *pars, std::map<string, double *> *funargs) {
  if (((*pars).isgroup)[sel[j]] == 0) {
    double tau   = *((*pars).tau);;
    (*priorgrad) = -2.0 / th[j] + th[j] / tau;
...
void pmomgzell_hess(double **hess, double *th, int *sel, int *thlength, struct marginalPars *pars,
                    std::map<string, double *> *funargs) {
...
*/
  ...
  double priorgrad, priorhess, *Sinv = (*funargs)["Sinv"], *nvaringroup = (*funargs)["nvarinselgroups"], *cholSini = (*
    funargs)["cholSini"];
...
void pemomgzell_log(double *f, double *th, int *sel, int *thlength, struct marginalPars *pars,
                    std::map<string, double *> *funargs) {
  demomgzell(f, th, (*pars).tau, (*funargs)["nvarinselgroups"], (*funargs)["nselgroups"], (*funargs)["ldetSinv"],
             (*funargs)["cholSinv"], (*funargs)["cholSini"], true);
  (*f) = -(*f);
}

void pemomgzell_gradhess(double *priorgrad, double *priorhess, int j, double *th, int *sel, int *thlength,
                         struct marginalPars *pars, std::map<string, double *> *funargs) {
...
void pemomgzell_grad(double *priorgrad, int j, double *th, int *sel, int *thlength, struct marginalPars *pars,
                     std::map<string, double *> *funargs) {
...
void pemomgzell_hess(double **hess, double *th, int *sel, int *thlength, struct marginalPars *pars,
                     std::map<string, double *> *funargs) {
...
void gzellgzell_log(double *f, double *th, int *sel, int *thlength, struct marginalPars *pars,
                    std::map<string, double *> *funargs) {
  dgzellgzell(f, th, (*funargs)["nvarinselgroups"], (*funargs)["nselgroups"], (*funargs)["ldetSinv"],
              (*funargs)["cholSinv"], (*funargs)["cholSini"], true);
  (*f) = -(*f);
}

void gzellgzell_gradhess(double *priorgrad, double *priorhess, int j, double *th, int *sel, int *thlength,
                         struct marginalPars *pars, std::map<string, double *> *funargs) {
...
void gzellgzell_grad(double *priorgrad, int j, double *th, int *sel, int *thlength, struct marginalPars *pars,
                     std::map<string, double *> *funargs) {
...
void gzellgzell_hess(double **hess, double *th, int *sel, int *thlength, struct marginalPars *pars,
                     std::map<string, double *> *funargs) {
...
void pmomgzellig_log(double *f, double *th, int *sel, int *thlength, struct marginalPars *pars,
                     std::map<string, double *> *funargs) {
  pmomgzell_log(f, th, sel, thlength, pars, funargs);
  (*f) -= dinvgammaC(exp(-2.0 * th[*thlength - 1]), *((*pars).alpha) / 2.0, *((*pars).lambda) / 2.0, 1) + log(2.0) - 2.0
      * th[*thlength - 1];
}

//Gradient and hessian wrt th[j] of log pMOM + group Zellner prior
void pmomgzellig_gradhess(double *priorgrad, double *priorhess, int j, double *th, int *sel, int *thlength,
                          struct marginalPars *pars, std::map<string, double *> *funargs) {
  if (j < *thlength - 1) {
    //if th[j] is a regression coefficient
    ...
    //if exp(th[j]) is the residual precision
...
//Gradient wrt th[j] of log pMOM + group Zellner prior
void pmomgzellig_grad(double *priorgrad, int j, double *th, int *sel, int *thlength, struct marginalPars *pars,
                      std::map<string, double *> *funargs) {
  if (j < *thlength - 1) {
    //if th[j] is a regression coefficient
...
void pmomgzellig_hess(double **hess, double *th, int *sel, int *thlength, struct marginalPars *pars,
                      std::map<string, double *> *funargs) {
  int j, k, kk, l, idxini, ngroups, ningroup, firstingroup;
  double priorgrad, priorhess, *Sinv = (*funargs)["Sinv"], *nvaringroup = (*funargs)["nvarinselgroups"], *cholSini = (*
    funargs)["cholSini"];
...
void pemomgzellig_log(double *f, double *th, int *sel, int *thlength, struct marginalPars *pars,
                      std::map<string, double *> *funargs) {
...
void pemomgzellig_gradhess(double *priorgrad, double *priorhess, int j, double *th, int *sel, int *thlength,
                           struct marginalPars *pars, std::map<string, double *> *funargs) {
  if (j < *thlength - 1) {
    //if th[j] is a regression coefficient
...
void pemomgzellig_grad(double *priorgrad, int j, double *th, int *sel, int *thlength, struct marginalPars *pars,
                       std::map<string, double *> *funargs) {
  if (j < *thlength - 1) {
    //if th[j] is a regression coefficient
...
void pemomgzellig_hess(double **hess, double *th, int *sel, int *thlength, struct marginalPars *pars,
                       std::map<string, double *> *funargs) {
  int j, k, kk, l, idxini, ngroups, ningroup, firstingroup;
  double priorgrad, priorhess, *Sinv = (*funargs)["Sinv"], *nvaringroup = (*funargs)["nvarinselgroups"], *cholSini = (*
    funargs)["cholSini"];
...
void gzellgzellig_log(double *f, double *th, int *sel, int *thlength, struct marginalPars *pars,
                      std::map<string, double *> *funargs) {
...
void gzellgzellig_gradhess(double *priorgrad, double *priorhess, int j, double *th, int *sel, int *thlength,
                           struct marginalPars *pars, std::map<string, double *> *funargs) {
  if (j < *thlength - 1) {
    //if th[j] is a regression coefficient
...
void gzellgzellig_grad(double *priorgrad, int j, double *th, int *sel, int *thlength, struct marginalPars *pars,
                       std::map<string, double *> *funargs) {
  if (j < *thlength - 1) {
    //if th[j] is a regression coefficient
...
void gzellgzellig_hess(double **hess, double *th, int *sel, int *thlength, struct marginalPars *pars,
                       std::map<string, double *> *funargs) {
...
void leastsquares(double *theta, double *phi, double *ypred, double *y, double *x, crossprodmat *XtX, double *ytX,
                  int *n, int *p, int *sel, int *nsel) {
  ...
  if ((*nsel) > 0) {
    //Least squares
    S    = dmatrix(1, *nsel, 1, *nsel);
    Sinv = dmatrix(1, *nsel, 1, *nsel);
    addct2XtX(&zero, XtX, sel, nsel, p, S);
    invdet_posdef(S, *nsel, Sinv, &detS);
    Asym_xsel(Sinv, *nsel, ytX, sel, theta);
    free_dmatrix(S, 1, *nsel, 1, *nsel);
    free_dmatrix(Sinv, 1, *nsel, 1, *nsel);
...
double pmomgzellMarg(int *sel, int *nsel, struct marginalPars *pars) {
  ...
  double num, den, ans = 0.0, aux, term1, *m, **S, **Sinv, **Vinv, **Vinv_chol, detS, detVinvtau, logdetVinv, tau = *(*
        pars).tau, tauinv = 1 / tau, taugroup = *(*pars).taugroup, taugroupinv = 1 / taugroup, logtaus, nuhalf,
      alphahalf =
          .5 * (*(*pars).alpha), lambdahalf = .5 * (*(*pars).lambda), ss, zero = 0, *nvarinselgroups, *firstingroup,
      nselgroups, *selgroups;
  ...
  if (*nsel == 0) {
    ...
    firstingroup    = dvector(0, min_xy(*nsel, *((*pars).ngroups)));
    selgroups       = dvector(0, *nsel - 1);
    findselgroups(nvarinselgroups, firstingroup, &nselgroups, selgroups, sel, nsel, (*pars).nvaringroup,
                  (*pars).ngroups); //copy subset of nvaringroup into nvarinselgroups
    m         = dvector(1, *nsel);
    S         = dmatrix(1, *nsel, 1, *nsel);
    Sinv      = dmatrix(1, *nsel, 1, *nsel);
    Vinv      = dmatrix(1, *nsel, 1, *nsel);
    Vinv_chol = dmatrix(1, *nsel, 1, *nsel);
    ...
    free_dmatrix(Vinv_chol, 1, *nsel, 1, *nsel);
  }
...
double pmomgmomMarg(int *sel, int *nsel, struct marginalPars *pars) {
  ...
  double num, den, ans = 0.0, aux, trSV, term1, *m, *mj, **S, **Sinv, **Vinv, **Vinv_chol, **Vinvj,
      ...
      taugroupinv = 1 / taugroup, logtaus, nuhalf, alphahalf = .5 * (*(*pars).alpha), lambdahalf =
          .5 * (*(*pars).lambda),
...
double zellgzellMarg(int *sel, int *nsel, struct marginalPars *pars) {
  ...
  double num, den, ans = 0.0, aux, term1, *m, **S, **Sinv, **Vinv, **Vinv_chol, detS, detVinvtau, logdetVinv, tau = *(*
    pars).tau, taugroup = *(*pars).taugroup, logtaus, nuhalf, alphahalf = .5 * (*(*pars).alpha), lambdahalf =
      .5 * (*(*pars).lambda), ss, zero = 0, *nvarinselgroups, *firstingroup, nselgroups, *selgroups;
  covariancemat *V0inv = (*pars).V0inv;
  bool posdef;
  if (*nsel == 0) {
    term1 = .5 * (*(*pars).n + *(*pars).alpha);
...
double bic_lm(int *sel, int *nsel, struct marginalPars *pars) {
  ...
  if (*nsel == 0) {
    phi = sumy2 / ((double) n);
    ans = -0.5 * (((double) n) + ((double) n) * log(2 * M_PI * phi)) + penalty;
    //Model dimension is 1 (variance parameter)
  } else {
    m     = dvector(1, *nsel);
    ypred = dvector(0, n - 1);
    ...
    ans = -0.5 * (ndouble + ndouble * log(2 * M_PI * phi) + ((double) *nsel + 1) * penalty);
    //Model dim is nsel + 1 (variance parameter)
...
double nlpMargAlapl(int *sel, int *nsel, struct marginalPars *pars, int *prior, int *symmetric) {
  ...
  thmode = dvector(1, p + fixedalpha);
  hess   = dmatrix(1, p + fixedalpha, 1, p + fixedalpha);
  ypred  = dvector(0, n - 1);

  postmodeAlaplCDA(thmode, &fmode, hess, sel, nsel, (*pars).n, (*pars).p, (*pars).y, (*pars).x, (*pars).XtX,
                   (*pars).ytX, &maxit, &ftol, &thtol, &taulapl, (*pars).taualpha, (*pars).fixatanhalpha, (*pars).alpha,
                   &lambdalapl, prior, hesstype, symmetric);
  ...
  if (method == 0) {
    ...
    thsim    = dvector(1, p + fixedalpha);
    cholV    = dmatrix(1, p, 1, p);
    cholVinv = dmatrix(1, p, 1, p);

    thmode[*nsel + 1] = log(thmode[*nsel + 1]);
    //if (*symmetric ==0) { thmode[p]= atanh(thmode[p]); }
    if ((*symmetric == 0) & (fixedalpha == 0)) { thmode[p] = atanh(thmode[p]); } else if (
      (*symmetric == 0) & (fixedalpha == 1)) { thmode[p + 1] = *((*pars).fixatanhalpha); }
    ...
    for (i = 1; i <= (*(*pars).B); i++) {
      ...
      fnegAlapl(&term1, ypred, thsim, sel, nsel, (*pars).n, (*pars).y, (*pars).x, &taulapl, (*pars).taualpha,
                (*pars).alpha, &lambdalapl, prior, true, symmetric, fixedalpha);
      term1 -= thsim[*nsel + 1];
    ...
    free_dvector(thsim, 1, p + fixedalpha);
    free_dmatrix(cholV, 1, p, 1, p);
    free_dmatrix(cholVinv, 1, p, 1, p);
  }
  ...
  free_dvector(thmode, 1, p + fixedalpha);
  free_dmatrix(hess, 1, p + fixedalpha, 1, p + fixedalpha);
  free_dvector(ypred, 0, n - 1);
  return (ans);
}


void postmodeAlaplCDA(double *thmode, double *fmode, double **hess, int *sel, int *nsel, int *n, int *pvar, double *y,
                      double *x, crossprodmat *XtX, double *ytX, int *maxit, double *ftol, double *thtol, double *tau,
                      double *taualpha, double *fixatanhalpha, double *alphaphi, double *lambdaphi, int *prior,
                      int *hesstype, int *symmetric) {
  bool useinit                  = false;
  ...
  if ((*symmetric == 0) & (fixedalpha == 0)) {
    ...
    if (*prior == 1) {
      ...
      if (thmode[p] > 0) { thmode[p] = thnew[p] = 0.5 * (thmode[p] + ss) / s; } else {
        thmode[p] = thnew[p] = 0.5 * (thmode[p] - ss) / s;
      }
    } else {
      //piMOM prior (also used for peMOM, although no longer exact)
      bool found;
      int root_count;
  ...
  while ((err > *thtol) & (it < (*maxit)) & (ferr > *ftol)) {
    err = ferr = 0;
    for (j = 1; j <= p; j++) {
      fpnegAlaplUniv(j, &g, &H, thmode, ypred, sel, nsel, n, pvar, y, x, XtX, tau, taualpha, alphaphi, lambdaphi, prior,
                     symmetric); //gradient and hessian
      if (j == *nsel + 1) g -= 1.0;
      delta    = g / H;
  ...
  fppnegAlapl(hess, thmode, ypred, sel, nsel, n, pvar, y, x, XtX, tau, taualpha, alphaphi, lambdaphi, prior, symmetric,
              hesstype); //Hessian

  thmode[*nsel + 1] = exp(thmode[*nsel + 1]);
  if ((*symmetric == 0) & (fixedalpha == 0)) { thmode[p] = tanh(thmode[p]); } else if (
    (*symmetric == 0) & (fixedalpha == 1)) { thmode[p + 1] = tanh(*fixatanhalpha); }
  //Note: tanh(z)= -1 + 2/(1+exp(-2*z))

  free_dvector(ypred, 0, *n - 1);
  free_dvector(thnew, 1, p + fixedalpha);
  free_dvector(fudgeh, 1, p);
}


void fppnegAlapl(double **H, double *th, double *ypred, int *sel, int *nsel, int *n, int *p, double *y, double *x,
                 crossprodmat *XtX, double *tau, double *taualpha, double *alphaphi, double *lambdaphi, int *prior,
                 int *symmetric, int *hesstype) {
...
void mleAlaplCDA(double *thmode, double *fmode, double *ypred, int *sel, int *nsel, int *n, int *p, double *y,
                 double *x, crossprodmat *XtX, double *ytX, int *maxit, bool useinit, int *symmetric,
  ...
  //Initialize
  thnew  = dvector(1, *nsel + 2);
  fudgeh = dvector(1, *nsel + 2);

  if ((*symmetric == 0) & (fixedalpha == 0)) {
    //if alpha must be estimated, init regression coef to median regression after 5 iter
  ...
  if ((*symmetric == 0) & (fixedalpha == 0)) { thmode[*nsel + 2] = thnew[*nsel + 2] = 0; } else if (
    (*symmetric == 0) & (fixedalpha == 1)) { thmode[*nsel + 2] = thnew[*nsel + 2] = *fixatanhalpha; }
  ...
  while ((err > 0.0001) && (ii < (*maxit))) {
    ...
    for (i = 0, s1 = s2 = 0; i < *n; i++) {
      if (y[i] < ypred[i]) { s1 += ypred[i] - y[i]; } else { s2 += y[i] - ypred[i]; }
    }
  ...
  free_dvector(thnew, 1, *nsel + 2);
  free_dvector(fudgeh, 1, *nsel + 2);
}


void fnegAlapl(double *ans, double *ypred, double *th, int *sel, int *nsel, int *n, double *y, double *x, double *tau,
               double *taualpha, double *alphaphi, double *lambdaphi, int *prior, bool logscale, int *symmetric,
               int fixedalpha) {
  ...
  if ((*prior) == 1) {
    if ((*symmetric == 0) & (fixedalpha == 0)) {
      if ((*nsel) > 0) {
        (*ans) += -dmomvec(th + 1, *nsel, 0.0, *tau, scale, 1, 1) - dmom(th[*nsel + 2], 0.0, *taualpha, 1.0, 1, 1) -
            dinvgammaC(scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      } else {
        (*ans) += -dmom(th[*nsel + 2], 0.0, *taualpha, 1.0, 1, 1) - dinvgammaC(
          scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      }
    } else {
      if ((*nsel) > 0) {
        (*ans) += -dmomvec(th + 1, *nsel, 0.0, *tau, scale, 1, 1) - dinvgammaC(
          scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      } else {
    ...
    if ((*symmetric == 0) & (fixedalpha == 0)) {
      if ((*nsel) > 0) {
        (*ans) += -dimomvec(th + 1, *nsel, 0.0, *tau, scale, 1) - dimom(th[*nsel + 2], 0.0, *taualpha, 1.0, 1) -
            dinvgammaC(scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      } else {
        (*ans) += -dimom(th[*nsel + 2], 0.0, *taualpha, 1.0, 1) - dinvgammaC(
          scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      }
    } else {
      if ((*nsel) > 0) {
        (*ans) += -dimomvec(th + 1, *nsel, 0.0, *tau, scale, 1) - dinvgammaC(
          scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      } else {
        (*ans) += -dinvgammaC(scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      }
    ...
    if ((*symmetric == 0) & (fixedalpha == 0)) {
      if ((*nsel) > 0) {
        (*ans) += -demomvec(th + 1, *nsel, *tau, scale, 1) - demom(th[*nsel + 2], *taualpha, 1.0, 1) - dinvgammaC(
          scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      } else {
        (*ans) += -demom(th[*nsel + 2], *taualpha, 1.0, 1) -
            dinvgammaC(scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      }
    } else {
      if ((*nsel) > 0) {
        (*ans) += -demomvec(th + 1, *nsel, *tau, scale, 1) -
            dinvgammaC(scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      } else {
        (*ans) += -dinvgammaC(scale, 0.5 * (*alphaphi), 0.5 * (*lambdaphi), 1);
      }
...
void fpnegAlaplUniv(int j, double *g, double *H, double *th, double *ypred, int *sel, int *nsel, int *n, int *p,
                    double *y, double *x, crossprodmat *XtX, double *tau, double *taualpha, double *alphaphi,
                    double *lambdaphi, int *prior, int *symmetric) {
  ...
  if ((*prior) == 1) {
    ...
    if (j <= (*nsel)) {
      ...
      gprior = 0.5 * (*nsel) - 0.5 * (*alphaphi) - 1.0 + 0.5 * (*lambdaphi) * exp(-th[*nsel + 1]) - exp(th[*nsel + 1]) *
               (*tau) * suminvth2;
    } else {
    ...
    if (j <= (*nsel)) {
      ...
      for (i = 1, sumth2 = 0, suminvth2 = 0; i <= (*nsel); i++) {
        sumth2 += th[i] * th[i];
        suminvth2 += 1.0 / (th[i] * th[i]);
      }
      gprior = -0.5 * (*nsel) - 0.5 * (*alphaphi) - 1.0 + 0.5 * (sumth2 / (*tau) + *lambdaphi) * exp(-th[*nsel + 1]) -
               exp(th[*nsel + 1]) * (*tau) * suminvth2;
    } else {
    ...
    if (j <= (*nsel)) {
      ...
      for (i = 1, sumth2 = 0, suminvth2 = 0; i <= (*nsel); i++) {
        sumth2 += pow(th[i], 2.0);
        suminvth2 += pow(1.0 / th[i], 2.0);
      }
      hprior = -0.5 * (*nsel) - 0.5 * (*alphaphi) - 1.0 + 0.5 * (sumth2 / (*tau) + (*lambdaphi)) * exp(-th[*nsel + 1]) -
               exp(th[*nsel + 1]) * (*tau) * suminvth2;
    } else {
...
void loglAlapl(double *ans, double *ypred, double *th, int *nsel, int *sel, int *n, double *scale, double *alpha,
               double *y, double *x, int *symmetric) {
...
void loglnegGradHessAlaplUniv(int j, double *g, double *H, double *th, int *nsel, int *sel, int *n, int *p, double *y,
                              double *ypred, double *x, crossprodmat *XtX, int *symmetric) {
  ...
  if (*symmetric == 0) {
    ...
    if (j < *nsel) {
      ...
      //derivative wrt vartheta

      for (i = 0; i < *n; i++) {
        if (y[i] < ypred[i]) { (*g) += w1 * (ypred[i] - y[i]); } else { (*g) += w2 * (y[i] - ypred[i]); }
      }
      (*H) = 0.25 * (*g) / sqscale;
      (*g) = (*n - (*g) / sqscale) * 0.5;
    } else {
      ...
      ws1    = exp(-2.0 * th[*nsel + 2]);
      ws2    = exp(2.0 * th[*nsel + 2]);
      wsbar1 = ws1;
      wsbar2 = -ws2;
      for (i = 0; i < *n; i++) {
        tmp = y[i] - ypred[i];
    ...
    if (j < *nsel) {
      ...
      for (i = 0; i < *n; i++) {
        if (y[i] < ypred[i]) { (*g) += (ypred[i] - y[i]); } else { (*g) += (y[i] - ypred[i]); }
      }
      (*H) = 0.25 * (*g) / sqscale;
      (*g) = (*n - (*g) / sqscale) * 0.5;
    }
...
void loglnegHessAlapl(double **H, double *th, int *nsel, int *sel, int *n, int *p, double *y, double *ypred, double *x,
                      crossprodmat *XtX, int *symmetric, int *hesstype) {
  int i, j, k, npar;
  double alpha, alphasq, scale, sqscale, wy0, sumwsy0, sumwsbary0, w1 = 1, w2 = 1, ws1, ws2, wsbar1, wsbar2, *Xtwbar, *
      Xtws, *hdiag, *D, *y0;
  scale   = exp(th[*nsel + 1]);
  sqscale = sqrt(scale);
  ...
  if (*symmetric == 0) {
    ...
    for (j = 0; j < *nsel; j++) {
      ...
      for (k = 0; k < j; k++) {
        H[j + 1][k + 1] = H[k + 1][j + 1] = (XtX->at(sel[k] * (*p) + sel[j])) / (scale * (1 - alphasq));
      }
      Xtwbar[j] = Xtws[j] = 0;
    }
    ...
    for (i = 0; i < *n; i++) {
      ...
      if (y[i] < ypred[i]) {
        ...
        for (j = 0; j < *nsel; j++) {
          Xtwbar[j] += w1 * x[sel[j] * (*n) + i];
          Xtws[j] += ws1 * x[sel[j] * (*n) + i];
        }
      } else {
        wy0 += w2 * y0[i];
        sumwsy0 += ws2 * y0[i];
        ...
        for (j = 0; j < *nsel; j++) {
          Xtwbar[j] -= w2 * x[sel[j] * (*n) + i];
          Xtws[j] += ws2 * x[sel[j] * (*n) + i];
        }
      }
    }
  ...
  if (*hesstype == 2) {
    ...
    free_dvector(hdiag, 1, npar);
    free_dvector(D, 1, npar);
  }
...
void quadapproxALaplace(double *hdiag, double **H, int *nsel, int *sel, int *n, double *y0, double *x, double *th,
                        double *vartheta, double *alpha, double *wy0, int *symmetric, double *w1, double *w2) {
...
double nlpMargSkewNorm(int *sel, int *nsel, struct marginalPars *pars, int *prior, int *symmetric) {
  ...
  thmode = dvector(1, p);
  hess   = dmatrix(1, p, 1, p);
  ypred  = dvector(0, n - 1);

  if ((*symmetric == 1) | (*((*pars).optimMethod) != 1)) {
    //Coordinate Descent Algorithm
    postmodeSkewNormCDA(thmode, &fmode, hess, sel, nsel, (*pars).n, (*pars).p, (*pars).y, (*pars).x, (*pars).XtX,
                        (*pars).ytX, &maxit, &ftol, &thtol, (*pars).tau, (*pars).taualpha, (*pars).alpha,
                        (*pars).lambda, prior, symmetric);
  } else {
    //LMA (modified Newton-Raphson)
    postmodeSkewNorm(thmode, &fmode, hess, sel, nsel, (*pars).n, (*pars).p, (*pars).y, (*pars).x, (*pars).XtX,
                     (*pars).ytX, &maxit, (*pars).tau, (*pars).taualpha, (*pars).alpha, (*pars).lambda, &initmle,
                     prior);
  }
  ...
  if (method == 0) {
    ...
    for (i = 1; i <= (*(*pars).B); i++) {
      rmvtC(thsim, p, thmode, cholV, nu);
      fnegSkewnorm(&term1, ypred, thsim, sel, nsel, (*pars).n, (*pars).y, (*pars).x, (*pars).XtX, (*pars).tau,
                   (*pars).taualpha, (*pars).alpha, (*pars).lambda, prior, true, symmetric);
      term1 -= thsim[*nsel + 1];
      term2 = -dmvtC(thsim, p, thmode, cholVinv, detVinv, nu, 1);
      ans += exp(-term1 + fmode + term2);
    }
  ...
  free_dvector(thmode, 1, p);
  free_dmatrix(hess, 1, p, 1, p);
  free_dvector(ypred, 0, n - 1);
  return (ans);
}


void postmodeSkewNorm(double *thmode, double *fmode, double **hess, int *sel, int *nsel, int *n, int *pvar, double *y,
                      double *x, crossprodmat *XtX, double *ytX, int *maxit, double *tau, double *taualpha,
                      double *alpha, double *lambda, bool *initmle, int *prior) {
  ...
  if (*initmle) {
    ...
    //Initialize at least-squares for theta; set (phi,alpha)= argmax likelihood for given theta
    ...
    pows1         = pow(s1, 1.0 / 3.0);
    pows2         = pow(s2, 1.0 / 3.0);
  ...
  if (ii == 1) {
    //LMA update not used in last iteration
    for (i = 1; i <= p; i++) {
      hess[i][i] = H[i][i];
      for (j = 1; j < i; j++) { hess[i][j] = hess[j][i] = H[i][j]; }
    }
  } else {
    //LMA update used, which modifies H
  ...
  free_dvector(ypred, 0, *n - 1);
  free_dvector(g, 1, p);
  free_dvector(delta, 1, p);
  free_dvector(thnew, 1, p);
  free_dmatrix(H, 1, p, 1, p);
  free_dmatrix(Hinv, 1, p, 1, p);
}


void postmodeSkewNormCDA(double *thmode, double *fmode, double **hess, int *sel, int *nsel, int *n, int *pvar,
                         double *y, double *x, crossprodmat *XtX, double *ytX, int *maxit, double *ftol, double *thtol,
                         double *tau, double *taualpha, double *alphaphi, double *lambdaphi, int *prior,
                         int *symmetric) {
  //Posterior mode for two-piece normal under pMOM, piMOM or peMOM prior on (theta,atanh(alpha)) and vartheta ~ IG(alpha/2,lambda/2)
  //
  ...
  while ((err > *thtol) & (it < (*maxit)) & (ferr > *ftol)) {
    ...
    for (j = 1; j <= p; j++) {
      if (j == *nsel + 1) {
        //update for phi
        if (*prior == 1) {
          //under MOM use exact max for phi
          ...
          fpnegSkewnormUniv(j, &g, thmode, ypred, sel, nsel, n, y, x, tau, taualpha, alphaphi, lambdaphi, prior,
                            symmetric); //gradient
          g -= 1.0;                     //jacobian term from tvartheta=log(tvartheta)
          fppnegSkewnormUniv(j, &H, thmode, ypred, sel, nsel, n, y, x, tau, taualpha, alphaphi, lambdaphi, prior,
                             symmetric); //Hessian
          delta    = g / H;
          thnew[j] = thmode[j] - delta;
        }
      } else {
        //update for theta, alpha
        fpnegSkewnormUniv(j, &g, thmode, ypred, sel, nsel, n, y, x, tau, taualpha, alphaphi, lambdaphi, prior,
                          symmetric); //gradient
        fppnegSkewnormUniv(j, &H, thmode, ypred, sel, nsel, n, y, x, tau, taualpha, alphaphi, lambdaphi, prior,
                           symmetric); //Hessian
  ...
  fppnegSkewnorm(hess, thmode, ypred, sel, nsel, n, y, x, tau, taualpha, alphaphi, lambdaphi, prior, symmetric);
  //Hessian
...
void fnegSkewnorm(double *ans, double *ypred, double *th, int *sel, int *nsel, int *n, double *y, double *x,
                  crossprodmat *XtX, double *tau, double *taualpha, double *alphaphi, double *lambdaphi, int *prior,
                  bool logscale, int *symmetric) {
...
void fpnegSkewnorm(double *g, double *th, double *ypred, int *sel, int *nsel, int *n, double *y, double *x, double *tau,
                   double *taualpha, double *alphaphi, double *lambdaphi, int *prior) {
...
void fpnegSkewnormUniv(int j, double *g, double *th, double *ypred, int *sel, int *nsel, int *n, double *y, double *x,
                       double *tau, double *taualpha, double *alphaphi, double *lambdaphi, int *prior, int *symmetric) {
  //Gradient of fnegSkewnorm
  //Note: the -1.0 term when j=nsel+1 coming from the jacobian of tvartheta=log(vartheta) is not included, it must be added separately after calling this function
  int i;
  ...
  if ((*prior) == 1) {
    ...
    if (j <= (*nsel)) {
      ...
      for (i = 1, suminvth2 = 0; i <= (*nsel); i++) { suminvth2 += 1.0 / (th[i] * th[i]); }
      gprior = 0.5 * (*nsel) - 0.5 * (*alphaphi) - 1 + 0.5 * (*lambdaphi) * exp(-th[*nsel + 1]) - exp(th[*nsel + 1]) * (
                 *tau) * suminvth2;
    } else {
      gprior = dimomgraduniv(th + (*nsel) + 2, &zero, taualpha);
    }
    ...
    if (j <= (*nsel)) {
      ...
      gprior = -0.5 * (*nsel) - 0.5 * (*alphaphi) - 1 + 0.5 * (sumth2 / (*tau) + *lambdaphi) * exp(-th[*nsel + 1]) -
               exp(th[*nsel + 1]) * (*tau) * suminvth2;
    } else {
      gprior = demomgraduniv(th + (*nsel) + 2, &zero, taualpha);
    }
...
void fppnegSkewnorm(double **H, double *th, double *ypred, int *sel, int *nsel, int *n, double *y, double *x,
                    double *tau, double *taualpha, double *alphaphi, double *lambdaphi, int *prior, int *symmetric) {
...
void fppnegSkewnormUniv(int j, double *H, double *th, double *ypred, int *sel, int *nsel, int *n, double *y, double *x,
                        double *tau, double *taualpha, double *alphaphi, double *lambdaphi, int *prior,
                        int *symmetric) {
  //Hessian of fnegSkewnorm
...
void loglSkewnorm(double *ans, double *ypred, double *th, int *nsel, int *sel, int *n, double *scale, double *alpha,
                  double *y, double *x, crossprodmat *XtX) {
...
void loglnegGradSkewNormUniv(int j, double *g, double *th, int *nsel, int *sel, int *n, double *y, double *ypred,
                             double *x, int *symmetric) {
...
void loglnegHessSkewNorm(double **H, double *th, int *nsel, int *sel, int *n, double *y, double *ypred, double *x,
  ...
  if ((*nsel) > 0) {
    ...
    for (i = 1; i <= (*nsel); i++) {
      H[i][*nsel + 1] = tX0Wy0[i] / sigma;
      H[*nsel + 1][i] = H[i][*nsel + 1];
    }
    ...
    if (*symmetric == 0) {
      ...
      for (i = 1; i <= (*nsel); i++) {
        H[i][*nsel + 2] = -tX0Wy0[i] / sigma;
        H[*nsel + 2][i] = H[i][*nsel + 2];
      }
    }
  ...
  free_dvector(Wy0, 0, *n - 1);
  free_dvector(Wsy0, 0, *n - 1);
}


void loglnegHessSkewNormUniv(int jj, double *H, double *th, int *nsel, int *sel, int *n, double *y, double *ypred,
                             double *x, int *symmetric) {
...
void mleSkewnorm(double *thmode, double *ypred, int *sel, int *nsel, int *n, int *p, double *y, double *x,
                 crossprodmat *XtX, double *ytX, int *maxit, bool useinit) {
  ...
  double *Xtwy, **XtwX, **XtwXinv, *thnew, err = 1.0, s1 = 0, s2 = 0, s1pow = 1, s2pow = 1, w, w1, w2, *e, *epred, *
      thuniv, difth1;
  ...
  if ((*nsel) > 0) {
    ...
    if (!useinit) {
      ...
      idxj   = (*n) * sel[0];
      for (i = 0; i < (*n); i++) {
        epred[i] = thmode[1] * x[i + idxj];
        e[i]     = y[i] - ypred[i] + epred[i];
      }
      thuniv[1] = thmode[1];
      thuniv[2] = thmode[*nsel + 1];
      thuniv[3] = thmode[*nsel + 2];
      mleSkewnorm(thuniv, epred, &seluniv, &nseluniv, n, p, e, x, XtX, ytX, &maxituniv, true);
      difth1            = thuniv[1] - thmode[1];
      thmode[1]         = thuniv[1];
      thmode[*nsel + 1] = thuniv[2];
      thmode[*nsel + 2] = thuniv[3];
      for (i = 0; i < (*n); i++) { ypred[i] += difth1 * x[i + idxj]; } //update linear predictor
      if ((*nsel) == 1) {
        //only 1 covariate, we're done
        err = -1;
        for (i = 0, s1 = 0, s2 = 0; i < *n; i++) {
          if (y[i] <= ypred[i]) { s1 += pow(y[i] - ypred[i], 2.0); } else { s2 += pow(y[i] - ypred[i], 2.0); }
        }
        s1pow             = pow(s1, 1.0 / 3.0);
        s2pow             = pow(s2, 1.0 / 3.0);
        thmode[*nsel + 2] = (s1pow - s2pow) / (s1pow + s2pow); //alpha
      }
      free_dvector(e, 0, *n - 1);
      free_dvector(epred, 0, *n - 1);
      free_dvector(thuniv, 1, 3);
    }
    ...
    while ((err > 0.0001) && (ii < (*maxit))) {
      ...
      w1 = 1.0 / pow(1.0 + thmode[*nsel + 2], 2.0);
      w2 = 1.0 / pow(1.0 - thmode[*nsel + 2], 2.0);
...
double f2opt_mom(double *th) {
  return fmomNegC_non0(th + 1, f2opt_pars.m + 1, f2opt_pars.S, f2opt_pars.phi, f2opt_pars.tau, f2opt_pars.r,
                       f2opt_pars.n, f2opt_pars.nsel);
}
...
double fmomNegC_non0(double *th, double *m, double **S, double *phi, double *tau, int *r, int *n, int *nsel) {
  ...
  z = dvector(0, *nsel);
  for (i = 0, sumlogth = 0; i < (*nsel); i++) {
    sumlogth += log(th[i] * th[i]);
    z[i] = th[i] - m[i];
  }
...
void momIntegralApproxC(double *ILaplace, double *thopt, double **Voptinv, double *fopt, int *n, int *nsel, double *m,
                        double **S, double *detS, double *phi, double *tau, int *r, int *logscale) {
  ...
  set_f2opt_pars(m, S, &emptydouble, XtX, &emptydouble, &emptydouble, &emptydouble, phi, tau, r, n, nsel, &emptyint,
                 nsel);
...
// [[Rcpp::export]]
SEXP pmomMarginalKI(SEXP Ssel, SEXP Snsel, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Ssumy2, SEXP SXtX, SEXP SytX, SEXP Sphi,
                    SEXP Stau, SEXP Sr, SEXP Smethod, SEXP SB, SEXP Slogscale, SEXP Sngroups, SEXP Snvaringroup) {
  struct marginalPars pars;
  ...
  set_marginalPars(&pars, &emptyint, INTEGER(Sn), INTEGER(Sn), INTEGER(Sp), REAL(Sy), &emptyint, REAL(Ssumy2),
                   &emptydouble, &emptydouble, &emptydouble, &emptydouble, XtX, REAL(SytX), INTEGER(Smethod), &emptyint,
                   &emptyint, &SoptimMethod, &Soptim_maxit, &usethinit, &emptydouble, INTEGER(SB), &emptydouble,
                   &emptydouble, &knownphi, REAL(Sphi), REAL(Stau), &emptydouble, taualpha, taualpha, INTEGER(Sr),NULL,
                   NULL,NULL,NULL, INTEGER(Slogscale), &offset,NULL,NULL, INTEGER(Sngroups),NULL,
                   INTEGER(Snvaringroup));
  PROTECT(ans = Rf_allocVector(REALSXP, 1));
  rans  = REAL(ans);
...
double pmomMarginalKC(int *sel, int *nsel, struct marginalPars *pars) {
  int i, j;
  double *m, s, **S, **Sinv, detS, num, den, logtau = log(*(*pars).tau), tauinv = 1.0 / (*(*pars).tau), logphi =
      log(*(*pars).phi), ans                        = 0.0, *thopt, **Voptinv, fopt;
  ...
  if (*nsel == 0) {
    ...
    den = .5 * ((*(*pars).n + .0) * (LOG_M_2PI + logphi) + log(detS) + (*nsel) * logtau) + (*nsel) * (*(*pars).r) * (
            logtau + logphi + ldoublefact(2 * (*(*pars).r) - 1));

    if ((*(*pars).method == 0) | ((*(*pars).method == -1) & ((*nsel) > 10))) {
      //Laplace

      thopt   = dvector(1, *nsel);
      Voptinv = dmatrix(1, *nsel, 1, *nsel);
      momIntegralApproxC(&ans, thopt, Voptinv, &fopt, (*pars).n, nsel, m, S, &detS, (*pars).phi, (*pars).tau, (*pars).r,
                         (*pars).logscale);
      free_dvector(thopt, 1, *nsel);
      free_dmatrix(Voptinv, 1, *nsel, 1, *nsel);
    } else if (*(*pars).method == 1) {
      //MC

      for (i = 1; i <= (*nsel); i++) {
        Sinv[i][i] = (*(*pars).phi) * Sinv[i][i];
        for (j = i + 1; j <= (*nsel); j++) { Sinv[i][j] = Sinv[j][i] = (*(*pars).phi) * Sinv[i][j]; }
      }
      ans = MC_mom_normal(m, Sinv, (*pars).r, nsel, (*pars).B);
    } else if (*(*pars).method == 2) {
      //Plug-in

      ans = rsumlogsq(m, (*pars).r, nsel);
    } else if ((*(*pars).method == -1) & ((*nsel) <= 10)) {
      //Exact

      Voptinv = dmatrix(1, *nsel, 1, *nsel);
      for (i = 1; i <= *nsel; i++)
        for (j          = i; j <= *nsel; j++)
          Voptinv[i][j] = Voptinv[j][i] = Sinv[i][j] * (*(*pars).phi);
...
// [[Rcpp::export]]
SEXP pmomMarginalUI(SEXP Ssel, SEXP Snsel, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Ssumy2, SEXP Sx, SEXP SXtX, SEXP SytX,
                    SEXP Stau, SEXP Sr, SEXP Smethod, SEXP SB, SEXP Slogscale, SEXP Salpha, SEXP Slambda, SEXP Sngroups,
                    SEXP Snvaringroup) {
  ...
  set_marginalPars(&pars, &emptyint, INTEGER(Sn), INTEGER(Sn), INTEGER(Sp), REAL(Sy), &emptyint, REAL(Ssumy2),
                   &emptydouble, &emptydouble, REAL(Sx), &emptydouble, XtX, REAL(SytX), INTEGER(Smethod), &emptyint,
                   &emptyint, &SoptimMethod, &Soptim_maxit, &usethinit, &emptydouble, INTEGER(SB), REAL(Salpha),
                   REAL(Slambda), &knownphi, &emptydouble, REAL(Stau), &emptydouble, taualpha, taualpha, INTEGER(Sr),
                   NULL,NULL,NULL,NULL, INTEGER(Slogscale), &offset,NULL,NULL, INTEGER(Sngroups),NULL,
                   INTEGER(Snvaringroup));
  PROTECT(ans = Rf_allocVector(REALSXP, 1));
  rans  = REAL(ans);
...
double pmomMarginalUC(int *sel, int *nsel, struct marginalPars *pars) {
  ...
  double num, den, ans = 0.0, term1, *m, **S, **Sinv, **Voptinv, detS, tau = *(*pars).tau, tauinv = 1.0 / tau, taugroup
      = *(*pars).taugroup, taugroupinv = 1.0 / taugroup, logtaus, nuhalf, alphahalf = .5 * (*(*pars).alpha), lambdahalf
      = .5 * (*(*pars).lambda), ss, zero = 0;
  bool hasgroups = (*((*pars).ngroups)) < (*((*pars).p));
  ...
  if (*nsel == 0) {
    ...
    if (*(*pars).method == 0) {
      ...
      if ((abs(tau - taugroup) > 1e-7) and hasgroups) {
        Rprintf(
          "Laplace method does not support different tau values between priorCoef and priorGroup\nUsing only tau from priorCoef");
      }
      ...
      den     = (*nsel) * ldoublefact(2 * (*(*pars).r) - 1.0) + .5 * (*(*pars).n * LOG_M_2PI + log(detS)) + logtaus +
            gamln(&alphahalf);
      ...
      if (*(*pars).method == 1) {
        ...
        for (i = 1; i <= *nsel; i++) {
          for (j = i; j <= *nsel; j++) { Sinv[i][j] = Sinv[j][i] = Sinv[i][j] * term1; }
        } //Vinv matrix
        ans = MC_mom_T(m, Sinv, &nu, (*pars).r, nsel, (*pars).B);
      } else if ((*(*pars).method == 2) | ((*(*pars).method == -1) & ((*nsel) > maxvarsexact))) {
        //Orthogonal approx
      ...
      } else if ((*(*pars).method == -1) & ((*nsel) <= maxvarsexact)) {
        //Exact

        Voptinv = dmatrix(1, *nsel, 1, *nsel);
        for (i = 1; i <= *nsel; i++)
          for (j          = i; j <= *nsel; j++)
            Voptinv[i][j] = Voptinv[j][i] = Sinv[i][j] * ss / (nu + .0);
...
double pmomMarginalUC_old(int *sel, int *nsel, struct marginalPars *pars) {
  int i, j, nu;
  double num, den, ans = 0.0, term1, *m, **S, **Sinv, detS, *thopt, **Voptinv, fopt, phiadj, tauinv =
      1.0 / (*(*pars).tau), nuhalf, alphahalf = .5 * (*(*pars).alpha), lambdahalf = .5 * (*(*pars).lambda), ss;
  if (*nsel == 0) {
    term1 = .5 * (*(*pars).n + *(*pars).alpha);
    num   = .5 * (*(*pars).alpha) * log(*(*pars).lambda) + gamln(&term1);
    den   = .5 * (*(*pars).n) * (LOG_M_PI) + gamln(&alphahalf);
    ...
    ss  = *(*pars).lambda + *(*pars).sumy2 - quadratic_xtAx(m, S, 1, *nsel);
    num = gamln(&nuhalf) + alphahalf * log(lambdahalf) + nuhalf * (log(2.0) - log(ss));
    den = (*nsel) * ldoublefact(2 * (*(*pars).r) - 1.0) + .5 * (*(*pars).n * LOG_M_2PI + log(detS)) + (*nsel) * (
            .5 + *(*pars).r) * log(*(*pars).tau) + gamln(&alphahalf);
    ...
    if ((*(*pars).method == 0) | ((*(*pars).method == -1) & ((*nsel) > 10))) {
      ...
      Voptinv = dmatrix(1, *nsel, 1, *nsel);
      phiadj  = (nu + .0) / (nu - 2.0);
      momIntegralApproxC(&ans, thopt, Voptinv, &fopt, (*pars).n, nsel, m, S, &detS, &phiadj, (*pars).tau, (*pars).r,
                         (*pars).logscale);
      free_dvector(thopt, 1, *nsel);
      free_dmatrix(Voptinv, 1, *nsel, 1, *nsel);
    } else if (*(*pars).method == 1) {
...
double fimomNegC(double *th, crossprodmat *XtX, double *ytX, double *phi, double *tau, int *n, int *p, int *sel,
                 int *nsel) {
  int i;
  ...
  ans = .5 * (quadratic_xseltAselxsel(th, XtX, p, nsel, sel) - 2 * ytXth) / (*phi) + (*tau) * (*phi) * suminvth +
        sumlogth;
  return ans;
}
...
void fppimomNegC_non0(double **ans, double *th, crossprodmat *XtX, double *ytX, double *phi, double *tau, int *n,
                      int *p, int *sel, int *nsel) {
  int i, j;
  double th2;
...
// [[Rcpp::export]]
SEXP pimomMarginalKI(SEXP Ssel, SEXP Snsel, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Ssumy2, SEXP SXtX, SEXP SytX, SEXP Sphi,
                     SEXP Stau, SEXP Smethod, SEXP SB, SEXP Slogscale, SEXP Sngroups, SEXP Snvaringroup) {
  int *sel = INTEGER(Ssel), *nsel = INTEGER(Snsel), *n = INTEGER(Sn), *p = INTEGER(Sp), *method = INTEGER(Smethod),
      SoptimMethod = 1, Soptim_maxit = -1, *B = INTEGER(SB), *logscale = INTEGER(Slogscale), r = 1, emptyint = 1,
      usethinit = 0, knownphi = 1;
  double *y = REAL(Sy), *sumy2 = REAL(Ssumy2), *ytX = REAL(SytX), *phi = REAL(Sphi), *tau = REAL(Stau), *rans,
      emptydouble = 0, offset = 0, *taualpha = NULL;
  struct marginalPars pars;
  crossprodmat *XtX;
  SEXP ans;

  XtX = new crossprodmat(REAL(SXtX), INTEGER(Sn)[0], INTEGER(Sp)[0], true);
  set_marginalPars(&pars, &emptyint, n, n, p, y, &emptyint, sumy2, &emptydouble, &emptydouble, &emptydouble,
                   &emptydouble, XtX, ytX, method, &emptyint, &emptyint, &SoptimMethod, &Soptim_maxit, &usethinit,
                   &emptydouble, B, &emptydouble, &emptydouble, &knownphi, phi, tau, &emptydouble, taualpha, taualpha,
                   &r,NULL,NULL,NULL,NULL, logscale, &offset,NULL,NULL, INTEGER(Sngroups),NULL, INTEGER(Snvaringroup));
  PROTECT(ans = Rf_allocVector(REALSXP, 1));
  rans  = REAL(ans);
...
double IS_imom(double *thopt, double **Voptinv, int *sel, int *nsel, int *n, int *p, crossprodmat *XtX, double *ytX,
  ...
  maxlogr = logr[0] = -fimomNegC_non0(thsim + 1, XtX, ytX, phi, tau, n, p, sel, nsel) - dmvtC(
                        thsim, *nsel, mprop, cholVpropinv, detVpropinv, 1, 1);
  for (i = 1; i < 1000; i++) {
    rmvtC(thsim, *nsel, mprop, cholVprop, 1);
    logr[i] = -fimomNegC_non0(thsim + 1, XtX, ytX, phi, tau, n, p, sel, nsel) - dmvtC(
                thsim, *nsel, mprop, cholVpropinv, detVpropinv, 1, 1);
  ...
  for (i = 1000; i < (*B); i++) {
    rmvtC(thsim, *nsel, mprop, cholVprop, 1);
    ans += exp(-fimomNegC_non0(thsim + 1, XtX, ytX, phi, tau, n, p, sel, nsel) - dmvtC(
                 thsim, *nsel, mprop, cholVpropinv, detVpropinv, 1, 1) - maxlogr + 500);
  }
...
double f2opt_imomU(double *th) {
  ...
  double ans;
  ans = fimomUNegC_non0(th + 1, f2opt_pars.sumy2, f2opt_pars.XtX, f2opt_pars.ytX, f2opt_pars.alpha, f2opt_pars.lambda,
                        f2opt_pars.tau, f2opt_pars.n, f2opt_pars.p, f2opt_pars.sel, f2opt_pars.nsel);
  return (ans);
}

double fimomUNegC_non0(double *th, double *sumy2, crossprodmat *XtX, double *ytX, double *alpha, double *lambda,
                       double *tau, int *n, int *p, int *sel, int *nsel) {
  ...
  ans = .5 * (*lambda + *sumy2 - 2 * ytXth + quadratic_xtAselx(th, XtX, p, nsel, sel)) / phi + (*tau) * phi * suminvth +
        sumlogth + .5 * eta * (*n - *nsel + *alpha);
  return ans;
}
...
// - Other arguments as in fimomNegC_non0
void fppimomUNegC_non0(double **ans, double *th, double *sumy2, crossprodmat *XtX, double *ytX, double *alpha,
                       double *lambda, double *tau, int *n, int *p, int *sel, int *nsel) {
  int i, j;
  ...
  ans[*nsel + 1][*nsel + 1] = .5 * (*lambda + *sumy2 - 2 * ytXth + quadratic_xtAselx(th + 1, XtX, p, nsel, sel)) / phi +
                              (*tau) * phi * suminvth;
  free_dvector(XtXth, 1, *nsel);
}

void imomModeU(double *th, PolynomialRootFinder::RootStatus_T *status, double *sumy2, crossprodmat *XtX, double *ytX,
               double *tau, double *alpha, double *lambda, int *sel, int *nsel, int *n, int *p) {
...
void imomUIntegralApproxC(double *ILaplace, double *thopt, int *sel, int *nsel, int *n, int *p, double *sumy2,
                          crossprodmat *XtX, double *ytX, double *alpha, double *lambda, double *tau, int *logscale,
                          int *hessian) {
  int iter, maxit = 100, emptyint;
  double ftol     = 1.0e-10, **dirth, **Vopt, **Voptinv, detVopt, emptydouble = 0, **emptymatrix, fopt;
  PolynomialRootFinder::RootStatus_T status;

  Vopt        = dmatrix(1, *nsel + 1, 1, *nsel + 1);
  Voptinv     = dmatrix(1, *nsel + 1, 1, *nsel + 1);
  dirth       = dmatrix(1, *nsel + 1, 1, *nsel + 1);
  emptymatrix = dmatrix(1, 1, 1, 1);
  //Initialize
  set_f2opt_pars(&emptydouble, emptymatrix, sumy2, XtX, ytX, alpha, lambda, &emptydouble, tau, &emptyint, n, p, sel,
                 nsel);
  ...
  free_dmatrix(Vopt, 1, *nsel + 1, 1, *nsel + 1);
  free_dmatrix(Voptinv, 1, *nsel + 1, 1, *nsel + 1);
  free_dmatrix(dirth, 1, *nsel + 1, 1, *nsel + 1);
  free_dmatrix(emptymatrix, 1, 1, 1, 1);
  if ((*logscale) != 1) { (*ILaplace) = exp(*ILaplace); }
}
...
double f2int_imom(double phi) {
  ...
  ans                   = exp(pimomMarginalKC(f2int_pars.sel, f2int_pars.nsel, &f2int_pars) + dinvgammaC(
              phi, .5 * (*f2int_pars.alpha), .5 * (*f2int_pars.lambda), 1) - *(f2int_pars.offset));
...
// [[Rcpp::export]]
SEXP pimomMarginalUI(SEXP Ssel, SEXP Snsel, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Ssumy2, SEXP Sx, SEXP SXtX, SEXP SytX,
                     SEXP Stau, SEXP Smethod, SEXP SB, SEXP Slogscale, SEXP Salpha, SEXP Slambda, SEXP Sngroups,
                     SEXP Snvaringroup) {
  int *sel = INTEGER(Ssel), *nsel = INTEGER(Snsel), *n = INTEGER(Sn), *p = INTEGER(Sp), *method = INTEGER(Smethod), *B =
      INTEGER(SB), *logscale = INTEGER(Slogscale), r = 1, SoptimMethod = 1, Soptim_maxit = -1, emptyint = 1, usethinit =
      0, knownphi = 0;
  double *y = REAL(Sy), *sumy2 = REAL(Ssumy2), *x = REAL(Sx), *ytX = REAL(SytX), *tau = REAL(Stau), *alpha =
      REAL(Salpha), *lambda = REAL(Slambda), *rans, emptydouble = 0, offset = 0, *taualpha = NULL;
  struct marginalPars pars;
  crossprodmat *XtX;
  SEXP ans;

  XtX = new crossprodmat(REAL(SXtX), INTEGER(Sn)[0], INTEGER(Sp)[0], true);
  set_marginalPars(&pars, &emptyint, n, n, p, y, &emptyint, sumy2, &emptydouble, &emptydouble, x, &emptydouble, XtX,
                   ytX, method, &emptyint, &emptyint, &SoptimMethod, &Soptim_maxit, &usethinit, &emptydouble, B, alpha,
                   lambda, &knownphi, &emptydouble, tau, &emptydouble, taualpha, taualpha, &r,NULL,NULL,NULL,NULL,
                   logscale, &offset,NULL,NULL, INTEGER(Sngroups),NULL, INTEGER(Snvaringroup));
  PROTECT(ans = Rf_allocVector(REALSXP, 1));
  rans  = REAL(ans);
...
double pimomMarginalUC(int *sel, int *nsel, struct marginalPars *pars) {
  ...
  double ans = 0, er, sumer2, **V, **Vinv, *thest, ypred, phiest, intmc, intlapl, *inputphi, num, den, term1, alphahalf
      = .5 * (*(*pars).alpha);
  ...
  if (*nsel == 0) {
    ...
    if ((*(*pars).method) == 0) {
      ...
      if ((*(*pars).method) == 2) {
        ...
        imomUIntegralApproxC(&ans, thest, sel, nsel, (*pars).n, (*pars).p, (*pars).sumy2, (*pars).XtX, (*pars).ytX,
                             (*pars).alpha, (*pars).lambda, (*pars).tau, &one, &hessian);
        ans = ans + alphahalf * log(.5 * (*(*pars).lambda)) - .5 * (*(*pars).n) * LOG_M_2PI - gamln(&alphahalf);
      } else if ((*(*pars).method) == 1) {
        //MC for each fixed phi + univariate integration
        set_f2int_pars((*pars).XtX, (*pars).ytX, (*pars).tau, (*pars).n, (*pars).p, sel, nsel, (*pars).y, (*pars).sumy2,
                       (*pars).method, (*pars).B, (*pars).alpha, (*pars).lambda, &zero);
        inputphi          = (*pars).phi;
        (*pars).phi       = &phiest;
        (*(*pars).method) = 0;
        inputlog          = (*pars).logscale;
        (*pars).logscale  = &one; //Laplace approx for phi=phiest
        intlapl           = pimomMarginalKC(sel, nsel, pars);
        (*pars).phi       = inputphi;
        (*(*pars).method) = 1;
        (*pars).logscale  = inputlog; //reset input values for phi, method
      ...
      } else if ((*(*pars).method) == 3) {
        //Hybrid Laplace - MC - Univariate integration
        set_f2int_pars((*pars).XtX, (*pars).ytX, (*pars).tau, (*pars).n, (*pars).p, sel, nsel, (*pars).y, (*pars).sumy2,
                       (*pars).method, (*pars).B, (*pars).alpha, (*pars).lambda, &zero);
        inputphi          = (*pars).phi;
        (*pars).phi       = &phiest;
        ...
        intlapl           = pimomMarginalKC(sel, nsel, pars);
        (*pars).phi       = inputphi;
        (*(*pars).method) = 2; //reset input values for phi, method
        if (intlapl == 0) {
          intmc += 1.0e-300;
          intlapl += 1.0e-300;
        ...
        f2int_pars.offset = &intlapl; //f2int_imom returns result divided by exp(intlapl) to avoid numerical overflow
        ans               = intmc + log(qromo(f2int_imom, 0.0, 100, midpnt) + qromo(f2int_imom, 100, 1.0e30, midinf));
        //adjusment is intmc - intlapl, but intlapl is the offset so needs to added back in
      }
      free_dmatrix(V, 1, *nsel, 1, *nsel);
      free_dmatrix(Vinv, 1, *nsel, 1, *nsel);
...
double zellnerMarginalUC(int *sel, int *nsel, struct marginalPars *pars) {
  int i, j;
  double num, den, ans = 0.0, term1, *m, **S, **Sinv, detS, adj, tau = *(*pars).tau, nuhalf, alphahalf =
      .5 * (*(*pars).alpha), lambdahalf = .5 * (*(*pars).lambda), ss, zero = 0;
  if (*nsel == 0) {
    term1 = .5 * (*(*pars).n + *(*pars).alpha);
    num   = .5 * (*(*pars).alpha) * log(*(*pars).lambda) + gamln(&term1);
    den   = .5 * (*(*pars).n) * (LOG_M_PI) + gamln(&alphahalf);
...
double normalidMarginalKC(int *sel, int *nsel, struct marginalPars *pars) {
  double *m, s, **S, **Sinv, detS, num, den, tau = *(*pars).tau, tauinv = 1 / tau, taugroup = *(*pars).taugroup,
      taugroupinv                                = 1 / taugroup, logphi = log(*(*pars).phi), ans = 0.0, zero = 0;
...
double normalidMarginalUC(int *sel, int *nsel, struct marginalPars *pars) {
  double num, den, ans = 0.0, term1, *m, **S, **Sinv, detS, tau = *(*pars).tau, tauinv = 1 / tau, taugroup = *(*pars).
      taugroup, taugroupinv = 1 / taugroup, nuhalf, alphahalf = .5 * (*(*pars).alpha), lambdahalf =
      .5 * (*(*pars).lambda), ss, zero = 0;
  ...
  if (*nsel == 0) {
    ...
    ss  = *(*pars).lambda + *(*pars).sumy2 - quadratic_xtAx(m, S, 1, *nsel);
    num = gamln(&nuhalf) + alphahalf * log(lambdahalf) + nuhalf * (log(2.0) - log(ss));
    den = .5 * (*(*pars).n * LOG_M_2PI + log(detS)) + .5 * (
            singlevarcount * log(tau) + (*nsel - singlevarcount) * log(taugroup)) + gamln(&alphahalf);
    ans = num - den;
