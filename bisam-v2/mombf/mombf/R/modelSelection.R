...
hasPostSampling <- function(object) {
  ...
  outcometype <- object$outcometype
  family <- object$family
  priorCoef <- object$prior$priorCoef@priorDistr
  priorGroup <- object$prior$priorGroup@priorDistr
...
setMethod("coefByModel", signature(object = "msfit"), function(object, maxmodels, alpha = 0.05, niter = 10^3, burnin = round(niter / 10)) {
  if (!is.null(object$postmean) & (object$prior$priorCoef@priorDistr %in% c("bic", "zellner"))) {
    ...
    ans <- vector("list", 3)
    names(ans) <- c("postmean", "ci.low", "ci.up")
...
predict.msfit <- function(object, newdata, data, level = 0.95, ...) {
  ...
  th <- rnlp(msfit = object, niter = 10^4)
  mx <- object$stdconstants[-1, "shift"]
  sx <- object$stdconstants[-1, "scale"]
  if (!missing(newdata)) {
...
modelSelection <- function(y, x, data, smoothterms, nknots = 9, groups = 1:ncol(x), constraints, center = TRUE, scale = TRUE, enumerate, includevars = rep(FALSE, ncol(x)), models, maxvars, niter = 5000, thinning = 1, burnin = round(niter / 10), family = "normal", priorCoef, priorGroup, priorDelta = modelbbprior(1, 1), priorConstraints, priorVar = igprior(.01, .01), priorSkew = momprior(tau = 0.348), phi, deltaini = rep(FALSE, ncol(x)), initSearch = "greedy", method = "auto", adj.overdisp = "intercept", hess = "asymp", optimMethod, optim_maxit, initpar = "none", B = 10^5, XtXprecomp = ifelse(ncol(x) < 10^4, TRUE, FALSE), verbose = TRUE) {
  ...
  if (missing(maxvars)){ #Jakob True bei uns! IS missing! 
    #Jakob family == "normal" -> p -> maxvars = p
  ...
  if (missing(priorGroup)) { #Jakob IS missing so execute code
  ...
  ngroups <- tmp$ngroups                #Jakob 21 (=p)
  constraints <- tmp$constraints        #Jakob 21 (=p)
  invconstraints <- tmp$invconstraints  #Jakob List of 21 zeros (always zeros???)
  nvaringroup <- tmp$nvaringroup        #Jakob Vec of 21 (=p) 1s
  groups <- tmp$groups                  #Jakob vec of numbers 0 to 20
  if (missing(models)) {                #Jakob models IS missing
    if (missing(enumerate)){
      enumerate <- ifelse(ngroups < 15, TRUE, FALSE) #Jakob Set to always false otherwise not using GibbsCI?
    }
  } else {
    enumerate <- TRUE
  }
  ...
  colnames(stdconstants) <- c("shift", "scale")
  ...
  usethinit <- thinit$usethinit
  thinit <- thinit$thinit
  ...
  optimMethod <- method$optimMethod
  optim_maxit <- method$optim_maxit
  adj.overdisp <- method$adj.overdisp
  hesstype <- method$hesstype
  ...
  familyint <- ffamily$familyint
  familygreedy <- ffamily$familygreedy
  if (familyint == 22) {
    sumlogyfact <- as.double(sum(lgamma(ystd + 1)))
  } else {
  ...
  priorgr <- tmp$priorgr
  tau <- tmp$tau
  taugroup <- tmp$taugroup
  alpha <- tmp$alpha
  lambda <- tmp$lambda
  taualpha <- tmp$taualpha
  fixatanhalpha <- tmp$fixatanhalpha
  priorCoef <- tmp$priorCoef
  priorGroup <- tmp$priorGroup
  ...
  prDelta <- tmp$prDelta
  prDeltap <- tmp$prDeltap
  parprDeltap <- tmp$parprDeltap
  prConstr <- tmp$prConstr
  prConstrp <- tmp$prConstrp
  parprConstrp <- tmp$parprConstrp
  ...
  if (!enumerate) {
    ...
    if (familyint == 0) {
      ...
      postMode <- rep(as.integer(0), p) #Jakob familyint == 1
    }
    ...
    if (initSearch == "greedy") {
      ...
      if (familyint == 0) {
        ...
        postModeProb <- as.double(postModeProb - 2 * log(2))
      }
      postMode[includevars == 1] <- TRUE
      ndeltaini <- as.integer(sum(postMode))
      deltaini <- as.integer(which(as.logical(postMode)) - 1)
    ...
    # --- ADDED BY JAKOB
    # Code to print out the state of every parameter before it goes into modelSelection cpp code
    parameter_names <- c(
      ...
      "hesstype", "optimMethod", "optim_maxit", "thinit", "usethinit", "B",
      "alpha", "lambda", "phi", "tau", "taugroup", "taualpha", "fixatanhalpha",
      "r", "prDelta", "prDeltap", "parprDeltap", "prConstr", "prConstrp",
    ...
    for (param_name in parameter_names) {
      if (exists(param_name)) {
        ...
        cat(paste0("Parameter: ", param_name, "\n"))
        cat("Content:\n")
        print(param_value)
        cat("\n")
      } else {
        cat(paste0("Parameter: ", param_name, " is not defined in the current environment.\n\n"))
      }
    }
    ...
    ans <- modelSelectionGibbsCI(postMode, postModeProb, knownphi, familyint, prior, priorgr, niter, 
                                 thinning, burnin, ndeltaini, deltaini, includevars, n, p, ystd, uncens, 
                                 sumy2, sumy, sumlogyfact, as.double(xstd), colsumsx, hasXtX, XtX, ytX, method, 
                                 adj.overdisp, hesstype, optimMethod, optim_maxit, thinit, usethinit, B, alpha, lambda, 
                                 phi, tau, taugroup, taualpha, fixatanhalpha, r, prDelta, prDeltap, parprDeltap, prConstr, 
                                 prConstrp, parprConstrp, groups, ngroups, nvaringroup, constraints, invconstraints, 
                                 as.integer(verbose))
    ...
    if (familyint == 0) {
      includeenum <- c(includevars[groups + 1], FALSE, FALSE)
    } else {
  ...
  if (length(uncens) > 0) {
    ...
    ystd <- Surv(time = ystd, event = uncens)
    xstd[ordery, ] <- xstd
  }
...
formatInputdata <- function(y, x, data, smoothterms, nknots, family) {
  ...
  if ("formula" %in% class(y)) {
    ...
    x <- des$x
    groups <- des$groups
    constraints <- des$constraints
    typeofvar <- des$typeofvar
    if ("Surv" %in% class(des$y)) {
      ...
      uncens <- as.integer(des$y[, 2])
      y <- des$y[, 1]
      ordery <- c(which(uncens == 1), which(uncens != 1))
      y <- y[ordery]
      x <- x[ordery, , drop = FALSE]
      uncens <- uncens[ordery]
...
createDesign <- function(formula, data, smoothterms, subset, na.action, splineDegree = 3, nknots = 14) {
  ...
  if (length(nn) > 0) { # there's interaction terms
    ...
    constraints <- lapply(1:ncol(groups2vars), function(i) {
      ans <- as.integer(which(groups2vars[, i] > 0))
      ans[ans != i] + intercept
    })
...
nestedSplines <- function(x, groups, smoothterms, data, subset, na.action, splineDegree, nknots) {
  ...
  for (kk in 1:length(nknots)) {
    ...
    groupsW[[kk]] <- integer(ncol(W[[kk]]))
    constraintsW[[kk]] <- vector("list", ncol(L))
    groupsL[[kk]] <- integer(ncol(L))
    constraintsL[[kk]] <- lapply(1:ncol(L), function(i) integer(0))
    selL <- rep(FALSE, ncol(L))
    nselL <- 0
    for (j in 1:ncol(L)) {
      m <- range(L[, j])
      tmp <- bspline(L[, j], degree = splineDegree, knots = seq(m[1], m[2], length = nknots[kk])) # equally-spaced knots
      Lj <- cbind(1, L[, j])
      b <- solve(t(Lj) %*% Lj) %*% (t(Lj) %*% tmp)
    ...
    if (any(varW < 1.0e-4)) {
      W[[kk]] <- W[[kk]][, varW > 1.0e-4]
      groupsW[[kk]] <- groupsW[[kk]][varW > 1.0e-4]
      constraintsW[[kk]] <- constraintsW[[kk]][unique(groupsW[[kk]] - min(groupsW[[kk]]) + 1)]
    }
  }

  # Combine design matrices hierarchically into single matrix with hierarchical constraints
  Wall <- W[[1]]
  groupsWall <- groupsW[[1]]
  constraintsWall <- constraintsW[[1]]
  ...
  if (length(nknots) == 1) {
    ...
    for (kk in 2:length(nknots)) {
      ...
      newgroups <- groupsW[[kk]][unlist(keep)]
      newgroups <- newgroups + maxgroups - min(newgroups) + 1
...
codeGroupsAndConstraints <- function(p, groups, constraints) {
  groupsnum <- as.numeric(factor(groups))
  groupsnum <- cumsum(c(TRUE, groupsnum[-1] != groupsnum[-length(groupsnum)])) # re-code groups (in order of appearance)
  ...
  if (missing(constraints)) {
    ...
    if (any(names(constraints) != as.character(g2code))) {
      ...
      for (i in 1:length(constraints)) {
        if (length(constraints[[i]] > 0)) constraints[[i]] <- as.integer(g2code[constraints[[i]]]) - as.integer(1)
      } # group codes start at 0
    } else {
  ...
  for (i in 1:ngroups) {
    invconstraints[[i]] <- as.integer(tabconstr[tabconstr[, "requires"] == (i - 1), "group"])
  }
...
formatmsMethod <- function(method, usethinit, initpar, optimMethod, optim_maxit = optim_maxit, priorCoef, priorGroup, knownphi, outcometype, family, hasgroups, adj.overdisp, hess) {
  ...
  if (outcometype == "glm") {
    if (optimMethod == "auto") {
      ...
      stop("Invalid optimMethod. For this family, only 'auto', 'Newton', 'LMA' or 'CDA' are implemented")
    }
  } else {
  ...
  if ((method == "ALA") && (usethinit == 3)) {
    ...
    if (missing(optim_maxit)) {
      optim_maxit <- as.integer(0)
    } else {
      optim_maxit <- as.integer(optim_maxit)
    } # maxit>0 tells C to use specified maxit
  } else {
    if (missing(optim_maxit)) {
      optim_maxit <- as.integer(-1)
    } else {
      optim_maxit <- as.integer(optim_maxit)
    } # maxit= -1 tells C to use its own defaults
  }
  ...
  if (method == "Laplace") {
    ...
    if (outcometype == "Continuous") {
      ...
      if (family == "normal") {
        ...
        stop("For survival outcomes, only family=='normal' currently implemented")
      }
    } else if (outcometype == "glm") {
      method <- as.integer(0)
    } else {
      stop("outcometype must be 'Continuous', 'Survival' or 'glm'")
    }
...
formatmsPriorsMarg <- function(priorCoef, priorGroup, priorVar, priorSkew, n) {
  ...
  if (priorCoef@priorDistr == "bic") { # FALSE
    ...
    alpha <- as.double(priorVar@priorPars["alpha"])
    lambda <- as.double(priorVar@priorPars["lambda"])
    #
...
formatmsPriorsModel <- function(priorDelta, priorConstraints, constraints) {
  ...
  } else if (priorDelta@priorDistr == "binomial") { #TRUE
    if ("p" %in% names(priorDelta@priorPars)) { # TRUE
  ...
  } else if (priorConstraints@priorDistr == "binomial") { # TRUE
    if ("p" %in% names(priorConstraints@priorPars)) { # TRUE
...
greedymodelSelectionR <- function(y, x, niter = 100, marginalFunction, priorFunction, betaBinPrior, deltaini = rep(FALSE, ncol(x)), verbose = TRUE, ...) {
  ...
  if (!missing(betaBinPrior)) {
    ...
    postOther <- matrix(NA, nrow = niter, ncol = 1)
    colnames(postOther) <- c("probBin")
  ...
  while (nchanges > 0 & itcur < niter) {
    ...
    for (i in 1:ncol(x)) {
      ...
      if (mnew > mcur) {
        ...
        mcur <- mnew
        nchanges <- nchanges + 1
        if (verbose) cat(paste(ifelse(sel[i], "Added", "Dropped"), nn[i], "\n", collapse = " "))
      }
    }
  }
  return(sel)
...
modelselBIC <- function(y, x, xadj, family, niter = 1000, burnin = round(.1 * niter), modelPrior, verbose = TRUE) {
  pluginJoint <- function(sel) {
    ...
    if (p > 0 & p <= length(y)) {
      ...
      ans$coef1 <- coef(glm1)[1:p]
      ans$coef2 <- coef(glm1)[-1:-p]
    } else if (p == 0) {
  ...
  for (j in 1:niter) {
    ...
    postModel[j, ] <- sel
    postCoef1[j, sel] <- curmod$coef1
    postCoef2[j, ] <- curmod$coef2
    if (verbose & ((j %% niter10) == 0)) cat(".")
  }
  if (verbose) cat("Done\n")
  # Return output
  if (burnin > 0) {
    postModel <- postModel[-1:-burnin, , drop = FALSE]
    postCoef1 <- postCoef1[-1:-burnin, , drop = FALSE]
    postCoef2 <- postCoef2[-1:-burnin, , drop = FALSE]
  }
...
nselConstraints <- function(sel, groups, constraints) {
  ...
  if (ngroupsconstr > 0) {
    for (i in which(hasconstraint)) {
      if (selgroup[i] & any(!selgroup[constraints[[i]]])) violateConstraint <- TRUE
    }
  }
  ngroups0 <- sum(selgroup[!hasconstraint])
  ngroups1 <- sum(selgroup[hasconstraint])
...
bbPriorTrunc <- function(sel, logscale = TRUE, maxvars = 10, ...) {
  ...
  if (sum(sel) <= maxvars) {
    ans <- bbPrior(sel, logscale = logscale, ...)
  } else {
    ans <- ifelse(logscale, -Inf, 0)
  }
  return(ans)
}
...
unifPriorTrunc <- function(sel, logscale = TRUE, maxvars = 10, ...) {
  ...
  if (sum(sel) <= maxvars) {
    ans <- unifPrior(sel, logscale = logscale, ...)
  } else {
    ans <- ifelse(logscale, -Inf, 0)
  }
  return(ans)
